...
The public ``chunk_gdn2`` entry dispatches to the fused TLE inference path
when enabled and otherwise uses the portable Triton implementation in
``gdn2_native``.
"""
...
from flag_attn.gated_delta_rule.triton_ops_helper import autotune_cache_kwargs
...
if HAS_TLE_GDN2:
    ...
    def _chunk_gdn2_fwd_intra_infer_kernel(
        ...
        for i_k in range(tl.cdiv(K, BK)):
            ...
            if USE_GATE_IN_KERNEL:
                if HAS_DT_BIAS:
                    p_dt = tl.make_block_ptr(dt_bias + i_h * K, (K,), (1,), (i_k * BK,), (BK,), (0,))
            ...
            p_g_out = tl.make_block_ptr(g_out, (T, K), (H * K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
        ...
        p_Akk = tl.make_block_ptr(Akk, (T, BT), (H * BT, 1), (i_t * BT, 0), (BT, BT), (1, 0))
        tl.store(p_Akk, b_Ai.to(Akk.dtype.element_ty), boundary_check=(0, 1))

        # Phase 3a: u_wy = inv(Akk) @ (write_w_gate * v)
        for i_v in range(tl.cdiv(V, BV)):
            p_v = tl.make_block_ptr(v, (T, V), (H * V, 1), (i_t * BT, i_v * BV), (BT, BV), (1, 0))
            p_write = tl.make_block_ptr(write_w_gate, (T, V), (H * V, 1), (i_t * BT, i_v * BV), (BT, BV), (1, 0))
        ...
        for i_k in range(tl.cdiv(K, BK)):
            p_k = tl.make_block_ptr(k, (T, K), (H * K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
            p_b = tl.make_block_ptr(b, (T, K), (H * K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
            p_gk = tl.make_block_ptr(g_out, (T, K), (H * K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
            ...
            p_w = tl.make_block_ptr(w_wy, (T, K), (H * K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
