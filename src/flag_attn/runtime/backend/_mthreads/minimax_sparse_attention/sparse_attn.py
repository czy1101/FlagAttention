...
# more head-level parallelism. The launch parameters below are the selected
# values after the num_warps and num_stages sweeps.
_PREFILL_BLOCK_H = 8
...
def _gqa_sparse_fwd_direct(
    ...
    pid_kh = pid_hg // NUM_HEAD_TILES
    pid_ht = pid_hg % NUM_HEAD_TILES
    head_start = pid_ht * BLOCK_SIZE_H
    ...
    for j in range(real_q_loop):
        ...
        q_ptrs = tl.make_block_ptr(
            ...
            offsets=(pid_q_j * BLOCK_SIZE_Q, head_start, 0),
...
def minimax_m3_sparse_attn(
    ...
    is_musa = q.device.type == "musa"
    grid = (max_query_len, num_kv_heads, batch)
    full_block_size_h = triton.next_power_of_2(gqa_group_size)
    if is_musa:
        block_size_h = min(_PREFILL_BLOCK_H, full_block_size_h)
    else:
        block_size_h = full_block_size_h
    num_head_tiles = triton.cdiv(gqa_group_size, block_size_h)
    ...
    # has an illegal WGMMA N dimension. The current MUSA TLE WGMMA backend also
    # rejects the transposed operands used by the optimized prefill kernel, so
    # use the portable Triton direct kernel on MUSA. CUDA keeps the existing
    # WGMMA fast path unchanged.
    use_direct = use_fp8 or block_size_h < 8 or is_musa
    if use_direct:
        direct_grid = (max_query_len, num_kv_heads * num_head_tiles, batch)
        _gqa_sparse_fwd_direct[direct_grid](
            ...
            num_warps=_PREFILL_NUM_WARPS if is_musa else 4,
            num_stages=_PREFILL_NUM_STAGES if is_musa else 3,
        )
        return
...
def minimax_m3_sparse_attn_decode(
    ...
    _gqa_sparse_decode_kernel[grid](
        ...
        KV_SCALE_MODE=kv_scale_mode,
        USE_PDL=use_pdl,
        # Tune only the main decode-attention kernel. Keep the merge kernel on
        # its existing default so this switch isolates the main-kernel effect.
