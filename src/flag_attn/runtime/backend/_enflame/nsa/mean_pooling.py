...
from flag_attn.parallel_nsa.triton_ops_helper import autotune_cache_kwargs
from flag_attn.parallel_nsa.utils import input_guard
...
def mean_pooling_fwd_kernel(
    ...
    i_d = tl.program_id(0)
    i_t = tl.program_id(1) + chunk_offset
...
def mean_pooling_fwd(
    ...
    for chunk_offset in range(0, NT, MAX_GRID_Y):
        chunk_count = min(
            MAX_GRID_Y,
            NT - chunk_offset,
        )
