...
def chunk_gdn2_fwd_intra(
    ...
    if bh > _ENFLAME_GRID_YZ_LIMIT:
        if NT > _ENFLAME_GRID_YZ_LIMIT:
            raise RuntimeError(
                "Enflame chunk_gdn2_fwd_intra grid requires both NT and B*H "
                "to fit within the y-axis limit "
                f"{_ENFLAME_GRID_YZ_LIMIT}; got NT={NT}, B*H={bh}."
            )
    ...
    # Delay loading the WY helper until the native path is executing.  This
    # keeps backend package initialization acyclic.
