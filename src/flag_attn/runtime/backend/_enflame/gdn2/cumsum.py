...
@triton.autotune(
    ...
    key=["B", "H", "S", "BT", "IS_VARLEN", "REVERSE", "GRID_MODE"],
)
@triton.jit(do_not_specialize=["T"])
...
def chunk_local_cumsum_vector(
    ...
    if NT > _ENFLAME_GRID_YZ_LIMIT:
        if bh > _ENFLAME_GRID_YZ_LIMIT:
            raise RuntimeError(
                "Enflame cumsum grid requires both NT and B*H to fit within "
                f"the y-axis limit {_ENFLAME_GRID_YZ_LIMIT}; got NT={NT}, B*H={bh}."
            )
    ...
    def grid(meta):
        n_s_blocks = triton.cdiv(meta["S"], meta["BS"])
        if grid_mode == _GRID_MODE_FLATTEN_T:
            return (n_s_blocks * NT, B * H, 1)
        if grid_mode == _GRID_MODE_FLATTEN_BH:
            return (n_s_blocks * B * H, NT, 1)
        return (n_s_blocks, NT, B * H)
