...
_GRID_MODE_FLATTEN_T = 1
_GRID_MODE_FLATTEN_BH = 2
...
@triton.autotune(
    ...
    key=["BT", "HV", "STATE_V_FIRST", "V", "GRID_MODE"],
)
@triton.jit(do_not_specialize=["T"])
def chunk_gla_fwd_kernel_o(
    ...
    if GRID_MODE == 1:
        i_flat = tl.program_id(0)
        i_v = i_flat % n_v_tiles
        i_t = i_flat // n_v_tiles
        i_bh = tl.program_id(1)
    elif GRID_MODE == 2:
        i_flat = tl.program_id(0)
        i_v = i_flat % n_v_tiles
        i_bh = i_flat // n_v_tiles
        i_t = tl.program_id(1)
    else:
        i_v = tl.program_id(0)
        i_t = tl.program_id(1)
...
if HAS_TLE:
    ...
    def chunk_gla_fwd_kernel_o_tle(
        ...
        n_vg_tiles = tl.cdiv(V, BV * B_VCHUNK)
        if GRID_MODE == 1:
            i_flat = tl.program_id(0)
            i_vg = i_flat % n_vg_tiles
            i_t = i_flat // n_vg_tiles
            i_bh = tl.program_id(1)
        elif GRID_MODE == 2:
            i_flat = tl.program_id(0)
            i_vg = i_flat % n_vg_tiles
            i_bh = i_flat // n_vg_tiles
            i_t = tl.program_id(1)
        else:
            i_vg = tl.program_id(0)
            i_t = tl.program_id(1)
            i_bh = tl.program_id(2)
...
def chunk_gla_fwd_o_gk(
    ...
    if NT > _ENFLAME_GRID_YZ_LIMIT:
        if bh > _ENFLAME_GRID_YZ_LIMIT:
            raise RuntimeError(
                "Enflame chunk_gla_fwd_o_gk grid requires both NT and B*HV "
                "to fit within the y-axis limit "
                f"{_ENFLAME_GRID_YZ_LIMIT}; got NT={NT}, B*HV={bh}."
            )
        grid_mode = _GRID_MODE_FLATTEN_T
    elif bh > _ENFLAME_GRID_YZ_LIMIT:
        grid_mode = _GRID_MODE_FLATTEN_BH

    def grid(meta):
        n_v_tiles = triton.cdiv(V, meta["BV"])
        if grid_mode == _GRID_MODE_FLATTEN_T:
            return (n_v_tiles * NT, B * HV, 1)
        if grid_mode == _GRID_MODE_FLATTEN_BH:
            return (n_v_tiles * B * HV, NT, 1)
        return (n_v_tiles, NT, B * HV)

    def grid_tle(meta):
        n_v_tiles = triton.cdiv(V, meta["BV"] * meta.get("B_VCHUNK", 1))
        if grid_mode == _GRID_MODE_FLATTEN_T:
            return (n_v_tiles * NT, B * HV, 1)
        if grid_mode == _GRID_MODE_FLATTEN_BH:
            return (n_v_tiles * B * HV, NT, 1)
    ...
    _use_tle = HAS_TLE and os.environ.get("FLAG_ATTN_GDN2_GLA_TLE", "1") != "0"
    if _use_tle:
