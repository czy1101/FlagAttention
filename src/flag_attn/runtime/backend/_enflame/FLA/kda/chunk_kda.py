...
"""Inference kernels for KDA prefill.

Three Triton forward paths with identical semantics share one public entry,
``chunk_kda``:

* Strict TLE path (``chunk_kda_fwd_infer_strict_tle``): optimized for the
  production KDA shape and its narrower input constraints.
* Generic TLE path (``chunk_kda_fwd_infer``): asynchronous TLE loads in K1
  plus an S60-compatible sequential state/output kernel for the wider input set.
...
``FLAG_ATTN_CHUNK_KDA_BACKEND`` to ``strict_tle``, ``tle``, or ``triton`` to
...
from ..compat import exp2, has_triton_tle
...
def _refresh_fp16_dot_precision() -> None:
    """S60 uses fixed IEEE dot precision."""


def _allocate_triton_workspace(size: int, _alignment: int, _stream) -> torch.Tensor:
    return torch.empty(size, device="gcu", dtype=torch.int8)


# =============================================================================
# Generic TLE path adapted for the Enflame S60 -- default when available
# =============================================================================
...
if HAS_TLE_KDA:
    ...
    def _kda_fwd_intra_kernel(
        ...
        # Keep the tile in registers on S60. The Enflame lowering cannot parse
        # the TLE shared-memory alloc/local_ptr dialect emitted by this path.
        b_gc = tl.cumsum(tl.where(m_c[:, None], b_g, 0.0), axis=0)

        p_g_out = tl.make_block_ptr(
            g_out, (T, K), (HV * K, 1), (i_t * BT, 0), (BT, K), (1, 0)
        )
        tl.store(
            p_g_out, b_gc.to(g_out.dtype.element_ty), boundary_check=(0, 1)
        )

        # Intra-chunk Aqk/Akk plus triangular solve.
        b_gq = tl.where(m_c[:, None], exp2(b_gc), 0.0)
        b_gk = tl.where(m_c[:, None], exp2(-b_gc), 0.0)
        ...
        b_gn = tl.sum(
            tl.where(o_i[:, None] == last_local, b_gc, 0.0), axis=0
        )[None, :]
    ...
    @triton.autotune(
        configs=[triton.Config({"BV": BV}, num_warps=4) for BV in [64]],
        key=["HV", "K", "V", "BT"],
    )
    @triton.jit(do_not_specialize=["T"])
    def _kda_fwd_state_output_s60_kernel(
        kg,
        v,
        beta,
        gk,
        ...
        """Generic S60 K2 without TLE pipe or TLE shared-memory objects."""
        # S60 limits grid.y to 255. Flatten the value-tile and sequence/head
        # dimensions into grid.x so large varlen batches (N * HV > 255) launch.
        i_pid = tl.program_id(0)
        n_v_tiles = tl.cdiv(V, BV)
        i_v = i_pid % n_v_tiles
        i_nh = i_pid // n_v_tiles
        i_n = i_nh // HV
        i_h = i_nh % HV
        ...
        if USE_INITIAL_STATE:
            if STATE_V_FIRST:
                ...
                b_h1 = tl.trans(tl.load(p_h0_1, boundary_check=(0, 1))).to(
                    tl.float32
                )
                if K > 64:
        ...
        for i_t in tl.range(NT):
            token_start = i_t * BT
            last_idx = tl.minimum(token_start + BT, T) - 1

            p_w1 = tl.make_block_ptr(
                ws, (T, 3 * K), (HV * 3 * K, 1), (token_start, 0), (BT, 64), (1, 0)
            )
            ...
            b_v_new_cast = b_v_new.to(kg.dtype.element_ty)
            b_o = scale * b_qh + tl.dot(b_Aqk, b_v_new_cast).to(tl.float32)
            ...
            b_h1 = b_h1 * exp2(b_gk1)[:, None] + tl.dot(
                tl.trans(b_kg1), b_v_new_cast
            ).to(tl.float32)
            if K > 64:
                b_h2 = b_h2 * exp2(b_gk2)[:, None] + tl.dot(
                    tl.trans(b_kg2), b_v_new_cast
                ).to(tl.float32)
            if K > 128:
                b_h3 = b_h3 * exp2(b_gk3)[:, None] + tl.dot(
                    tl.trans(b_kg3), b_v_new_cast
                ).to(tl.float32)
            if K > 192:
                b_h4 = b_h4 * exp2(b_gk4)[:, None] + tl.dot(
                    tl.trans(b_kg4), b_v_new_cast
                ).to(tl.float32)
...
def chunk_kda_fwd_infer(
    ...
    **kwargs,
    ...
    # The generic TLE entry covers wider K/V/GVA/layout combinations, but its
    # legacy kernels split K into 64-wide state tiles and keep the small
    # triangular inverse inside K1.  For the production-compatible subset,
    # reuse the independently validated S60 core kernels while retaining this
    # generic host entry.  Inputs outside that subset continue through the
    # wider generic kernels below.
    core_reason = strict_tle_input_error(
        q=q,
        k=k,
        v=v,
        g=g,
...
def strict_tle_input_error(
    ...
    chunk_size: int,
    require_tle: bool = True,
) -> str | None:
    if require_tle and not HAS_STRICT_TLE_KDA:
    ...
    if any(tensor.device.type != "gcu" for tensor in inputs.values()):
        return "strict TLE KDA requires GCU inputs"
    ...
    if chunk_size not in (16, 32):
        return f"strict TLE KDA requires chunk_size in {{16, 32}}, got {chunk_size}"
    ...
    if cu_seqlens is not None:
        ...
        if (
            ...
            return (
                "strict TLE KDA requires a 1D int32 cu_seqlens tensor on the "
                "input device"
            )
...
if HAS_STRICT_TLE_KDA:
    ...
    def _strict_kda_fwd_intra_kernel(
        ...
        k += (bos * H + i_h) * K
        g += (bos * HV + i_hv) * K
        g_last += (i_tg * HV + i_hv).to(tl.int32) * K
        if IS_VARLEN:
            a_chunk = i_hv * NT_TOTAL + i_tg
        else:
            a_chunk = (i_b * HV + i_hv) * NT_TOTAL + i_t
        Aqk += a_chunk.to(tl.int32) * BT * BT
        Akk_raw += a_chunk.to(tl.int32) * BT * BT
        Akk += a_chunk.to(tl.int32) * BT * BT
        ws += (bos * HV + i_hv) * 3 * K
        beta += bos * HV + i_hv
        ...
        b_q = tle.load(p_q, boundary_check=(0, 1), is_async=True)
        b_k = tle.load(p_k, boundary_check=(0, 1), is_async=True)
        b_q = tl.where(m_c[:, None], b_q, 0.0)
        b_k = tl.where(m_c[:, None], b_k, 0.0)
        ...
        if K1_DIAG_MODE < 15:
            ...
            b_gc = tl.zeros([BT, K], dtype=tl.float32)

        if K1_DIAG_MODE < 3:
            # Aqk and Akk share the same gated-k tile. For BT=16, compute
            # their transposes together so GCU300 sees one N=32 matrix
            # primitive instead of two poorly utilized N=16 primitives.
            if BT == 16:
                b_gq = tl.where(m_c[:, None], exp2(b_gc), 0.0)
                b_gk = tl.where(m_c[:, None], exp2(-b_gc), 0.0)
                b_k_left = (b_kf * b_gk).to(b_k.dtype)
                b_q_right = tl.trans((b_qf * b_gq).to(b_q.dtype))
                b_k_right = tl.trans((b_kf * b_gq).to(b_k.dtype))
                b_qk_right = tl.reshape(
                    tl.join(b_q_right, b_k_right), (K, 2 * BT)
                )
                b_Aqk_Akk_t = tl.dot(
                    b_k_left, b_qk_right, out_dtype=tl.float32
                )
                b_Aqk_t, b_Akk_t = tl.split(
                    tl.reshape(b_Aqk_Akk_t, (BT, BT, 2))
                )
            else:
                # With 32 rows, exp2(-b_gc) can exceed the fp32/bf16
                # exponent range even though every retained triangular
                # product is finite. Shift both operands by the cumulative
                # gate at the 16-row boundary, then form only the three
                # blocks that survive the lower-triangular output mask. The
                # omitted bottom-left transpose block is both unused and the
                # one whose positive exponent difference can overflow.
                split_row: tl.constexpr = BT // 2
                b_shift = tl.sum(
                    tl.where(
                        o_i[:, None] == split_row - 1,
                        b_gc,
                        0.0,
                    ),
                    axis=0,
                ...
                b_gq = tl.where(
                    m_c[:, None], exp2(b_gc - b_shift), 0.0
                )
                b_gk = tl.where(
                    m_c[:, None], exp2(b_shift - b_gc), 0.0
                )
                b_k_left = (b_kf * b_gk).to(b_k.dtype)
                b_q_scaled = (b_qf * b_gq).to(b_q.dtype)
                b_k_scaled = (b_kf * b_gq).to(b_k.dtype)
                m_lo = o_i < split_row
                b_k_left_lo = tl.where(m_lo[:, None], b_k_left, 0.0)
                b_k_left_hi = tl.where(~m_lo[:, None], b_k_left, 0.0)
                b_q_right_lo = tl.trans(
                    tl.where(m_lo[:, None], b_q_scaled, 0.0)
                )
                b_q_right_hi = tl.trans(
                    tl.where(~m_lo[:, None], b_q_scaled, 0.0)
                )
                b_k_right_lo = tl.trans(
                    tl.where(m_lo[:, None], b_k_scaled, 0.0)
                )
                b_k_right_hi = tl.trans(
                    tl.where(~m_lo[:, None], b_k_scaled, 0.0)
                )

                b_Aqk_t = tl.dot(
                    b_k_left_lo, b_q_right_lo, out_dtype=tl.float32
                )
                b_Aqk_t += tl.dot(
                    b_k_left_lo, b_q_right_hi, out_dtype=tl.float32
                )
                b_Aqk_t += tl.dot(
                    b_k_left_hi, b_q_right_hi, out_dtype=tl.float32
                )
                b_Akk_t = tl.dot(
                    b_k_left_lo, b_k_right_lo, out_dtype=tl.float32
                )
                b_Akk_t += tl.dot(
                    b_k_left_lo, b_k_right_hi, out_dtype=tl.float32
                )
                b_Akk_t += tl.dot(
                    b_k_left_hi, b_k_right_hi, out_dtype=tl.float32
                )
            b_Aqk = tl.trans(b_Aqk_t)
            b_Akk = tl.trans(b_Akk_t)
        ...
        if K1_DIAG_MODE < 7:
            ...
            b_beta = tl.where(m_c, b_beta, 0.0)
        else:
            b_beta = tl.zeros([BT], dtype=tl.float32)

        m_valid = m_c[:, None] & m_c[None, :]
        m_Aqk = (o_i[:, None] >= o_i[None, :]) & m_valid
        m_Akk = (o_i[:, None] > o_i[None, :]) & m_valid
        m_I = o_i[:, None] == o_i[None, :]
        ...
        if K1_DIAG_MODE < 7:
            # Finish every use of the large q/k/g tiles before the triangular
            # inverse below. On S60 these tiles live in registers because the
            # current lowering cannot parse the TLE shared-memory/local_ptr
            # objects used by the Hopper path.
            b_q3 = b_qf * b_q_rstd[:, None]
            b_qg_val = b_q3 * exp2(b_gc)
            p_qg = tl.make_block_ptr(
                ws,
                (T, 3 * K),
                (HV * 3 * K, 1),
            ...
            tl.store(
                p_qg, b_qg_val.to(ws.dtype.element_ty), boundary_check=(0, 1)
            )

            b_k3 = b_kf * b_k_rstd[:, None]
            b_kb = b_k3 * b_beta[:, None] * exp2(b_gc)
            p_w = tl.make_block_ptr(
                ws,
                (T, 3 * K),
                (HV * 3 * K, 1),
            ...
            tl.store(
                p_w, b_kb.to(ws.dtype.element_ty), boundary_check=(0, 1)
            )
        ...
        p_g_last = tl.make_block_ptr(
            g_last, (1, K), (K, 1), (0, 0), (1, K), (1, 0)
        )
        tl.store(p_g_last, b_gn.to(g_last.dtype.element_ty), boundary_check=(0, 1))
        if K1_DIAG_MODE < 7:
            b_kg_val = b_k3 * tl.where(m_c[:, None], exp2(b_gn - b_gc), 0)
            p_kg = tl.make_block_ptr(
                ws,
                (T, 3 * K),
                (HV * 3 * K, 1),
            ...
            tl.store(
                p_kg, b_kg_val.to(ws.dtype.element_ty), boundary_check=(0, 1)
            )

        if K1_DIAG_MODE < 1 and BT != 16:
            # qf/kf/gc and their large derived tiles are dead at this point,
            # so only the small triangular matrices remain live.
            b_L = b_Akk.to(tl.float16)
            b_Ai = m_I.to(tl.float16) - b_L
            b_L2 = tl.dot(b_L, b_L, out_dtype=tl.float16)
            b_Ai = b_Ai + tl.dot(b_Ai, b_L2, out_dtype=tl.float16)
            ...
            b_Ai = b_Ai + tl.dot(b_Ai, b_L8, out_dtype=tl.float16)
            if BT == 32:
                # (I-L)(I+L^2)(I+L^4)(I+L^8)(I+L^16)
                # equals I-L^32, which is I for a strictly lower 32x32 L.
                b_L16 = tl.dot(b_L8, b_L8, out_dtype=tl.float16)
                b_Ai = b_Ai + tl.dot(b_Ai, b_L16, out_dtype=tl.float16)
        else:
            b_Ai = b_Akk.to(tl.float16)

        if K1_DIAG_MODE < 1 and BT == 16:
            p_Akk_raw = tl.make_block_ptr(
                Akk_raw, (BT, BT), (BT, 1), (0, 0), (BT, BT), (1, 0)
            )
            tl.store(p_Akk_raw, b_Ai)
        else:
            p_Akk_out = tl.make_block_ptr(
                Akk, (BT, BT), (BT, 1), (0, 0), (BT, BT), (1, 0)
            )
            tl.store(p_Akk_out, b_Ai.to(Akk.dtype.element_ty))

    @triton.jit
    def _strict_kda_inverse_pair_s60_kernel(
        Akk_raw,
        Akk,
        N_MATRICES,
    ):
        """Invert two independent 16x16 systems as one 32x32 block diagonal."""
        i_pair = tl.program_id(0)
        o_i = tl.arange(0, 32)
        o_j = tl.arange(0, 32)
        row_group = o_i[:, None] // 16
        col_group = o_j[None, :] // 16
        row = o_i[:, None] % 16
        col = o_j[None, :] % 16
        same_block = row_group == col_group

        matrix_id = 2 * i_pair + row_group
        matrix_valid = matrix_id < N_MATRICES
        offsets = matrix_id * 16 * 16 + row * 16 + col
        b_L = tl.load(
            Akk_raw + offsets,
            mask=same_block & matrix_valid,
            other=0.0,
        )

        b_I = same_block & matrix_valid & (row == col)
        b_Ai = b_I.to(tl.float16) - b_L
        b_L2 = tl.dot(b_L, b_L, out_dtype=tl.float16)
        b_Ai = b_Ai + tl.dot(b_Ai, b_L2, out_dtype=tl.float16)
        b_L4 = tl.dot(b_L2, b_L2, out_dtype=tl.float16)
        b_Ai = b_Ai + tl.dot(b_Ai, b_L4, out_dtype=tl.float16)
    ...
    def _strict_kda_fwd_intra(
        ...
        if diagnostic_mode < 1 and BT == 16:
            Akk_raw = torch.empty(
                B, HV, NT, BT, BT, device=q.device, dtype=torch.float16
            )
        else:
            Akk_raw = Akk
        ...
        _strict_kda_fwd_intra_kernel[grid](
            ...
            K=K,
            BT=BT,
            K1_DIAG_MODE=diagnostic_mode,
        )
        if diagnostic_mode < 1 and BT == 16:
            n_matrices = B * HV * NT
            inverse_grid = (triton.cdiv(n_matrices, 2),)
            _strict_kda_inverse_pair_s60_kernel[inverse_grid](
                Akk_raw=Akk_raw,
                Akk=Akk,
    ...
    @triton.jit(do_not_specialize=["T"])
    def _strict_kda_fwd_state_output_s60_kernel(
        v,
        beta,
        gk,
        Aqk,
        ...
        STORE_FINAL_STATE: tl.constexpr,
        IS_VARLEN: tl.constexpr,
        K2_DIAG_MODE: tl.constexpr,
    ):
        """S60 strict K2 without TLE pipe/warp-specialization dialects."""
        # Keep the launch one-dimensional: S60 grid.y is limited to 255, while
        # benchmark varlen cases can have N * HV well above that limit.
        i_pid = tl.program_id(0)
        n_v_tiles = tl.cdiv(V, BV)
        i_v = i_pid % n_v_tiles
        i_nh = i_pid // n_v_tiles
        ...
        if IS_VARLEN:
            ...
            NT = tl.cdiv(T, BT)
            chunk_start = tl.load(chunk_offsets + i_n).to(tl.int32)
            a_chunk_start = i_h * NT_TOTAL + chunk_start
        else:
            bos = i_n * T
            NT = tl.cdiv(T, BT)
            chunk_start = i_n * NT
            a_chunk_start = (i_n * HV + i_h) * NT_TOTAL

        v += (bos * HV + i_h) * V
        beta += bos * HV + i_h
        o += (bos * HV + i_h) * V
        ws += (bos * HV + i_h) * 3 * K
        gk += (chunk_start * HV + i_h) * K
        Aqk += a_chunk_start * BT * BT
        Akk += a_chunk_start * BT * BT
        ...
        for i_t in tl.range(NT):
            ...
            b_v_raw = tl.load(p_v, boundary_check=(0, 1))
            b_v_raw = tl.where(m_t[:, None], b_v_raw, 0.0)
            b_beta = tl.load(p_beta, boundary_check=(0,)).to(tl.float32)
            b_beta = tl.where(m_t, b_beta, 0.0)
            # Match the original strict producer: beta-scaled v is rounded to
            # bf16 before the MMA consumer forms the residual.
            b_vb = (b_v_raw.to(tl.float32) * tl.sigmoid(b_beta)[:, None]).to(
                tl.bfloat16
            ).to(tl.float32)
            if K2_DIAG_MODE != 4:
                b_Akk = tl.load(p_Akk)

            b_h_bf = b_h.to(tl.bfloat16)
            if K2_DIAG_MODE != 8:
                b_w = tl.load(p_w, boundary_check=(0, 1))
                b_w = tl.where(m_t[:, None], b_w, 0.0)
                b_kh = tl.dot(b_w, b_h_bf, out_dtype=tl.float32)
            else:
                b_kh = tl.zeros([BT, BV], dtype=tl.float32)
            b_diff = b_vb - b_kh
            if K2_DIAG_MODE != 4:
                b_v_new = tl.dot(
                    b_Akk, b_diff.to(tl.bfloat16), out_dtype=tl.float32
                )
            else:
                b_v_new = b_diff
            b_v_new = tl.where(m_t[:, None], b_v_new, 0.0)
            b_v_new_bf = b_v_new.to(tl.bfloat16)

            # Output phase. Defer qg and Aqk until the residual phase is
            # complete so their tiles do not overlap the first projection.
            if K2_DIAG_MODE != 16:
                p_qg = tl.make_block_ptr(
                    ws,
                    (T, 3 * K),
                ...
                b_qg = tl.where(m_t[:, None], b_qg, 0.0)
                b_qh = tl.dot(b_qg, b_h_bf, out_dtype=tl.float32)
            else:
                b_qh = tl.zeros([BT, BV], dtype=tl.float32)
            b_o = scale * b_qh
            if K2_DIAG_MODE != 2:
                p_Aqk = tl.make_block_ptr(
                    Aqk + i_t * BT * BT,
                ...
                b_Aqk = tl.load(p_Aqk)
                b_o += tl.dot(b_Aqk, b_v_new_bf, out_dtype=tl.float32)
            p_o = tl.make_block_ptr(
                o,
                (T, V),
                (HV * V, 1),
            ...
            # State-update phase. kg is the largest remaining input tile, so
            # defer its load until the output tiles are no longer needed.
            if K2_DIAG_MODE != 32:
                o_k = tl.arange(0, K)
                b_gk = tl.load(
                    gk + i_t * HV * K + o_k, mask=o_k < K, other=0.0
                ).to(tl.float32)
                b_h_next = b_h * exp2(b_gk)[:, None]
            else:
                b_h_next = b_h
            ...
            if K2_DIAG_MODE != 1:
                ...
                b_kg = tl.where(m_t[:, None], b_kg, 0.0)
                b_h_next += tl.dot(
                    tl.trans(b_kg), b_v_new_bf, out_dtype=tl.float32
                )
            b_h = b_h_next
    ...
    def _strict_kda_fwd_state_output(
        ...
        BV = 128
        _strict_kda_fwd_state_output_s60_kernel[
...
@triton.jit(do_not_specialize=["T"])
def _kda_fwd_intra_core_triton_kernel(
    q,
    k,
    g,
    beta,
    ...
    """TLE-free K1 for the S60 K=V=128, BT=32 production shape."""
    i_tg, i_bh = tl.program_id(0), tl.program_id(1)
    i_t = i_tg
    i_hv = i_bh % HV
    i_h = i_hv // (HV // H)
    ...
    # A single exponent shift keeps every operand representable. Only the
    # three blocks retained by the lower-triangular mask are formed, avoiding
    # the unused overflow-prone quadrant of the BT32 product.
    split_row: tl.constexpr = BT // 2
    b_shift = tl.sum(
        tl.where(o_i[:, None] == split_row - 1, b_gc, 0.0), axis=0
    )[None, :]
    b_gq = tl.where(m_c[:, None], exp2(b_gc - b_shift), 0.0)
    b_gk = tl.where(m_c[:, None], exp2(b_shift - b_gc), 0.0)
    b_k_left = (b_kf * b_gk).to(b_k.dtype)
    b_q_scaled = (b_qf * b_gq).to(b_q.dtype)
    b_k_scaled = (b_kf * b_gq).to(b_k.dtype)
    m_lo = o_i < split_row
    b_k_left_lo = tl.where(m_lo[:, None], b_k_left, 0.0)
    b_k_left_hi = tl.where(~m_lo[:, None], b_k_left, 0.0)
    b_q_right_lo = tl.trans(tl.where(m_lo[:, None], b_q_scaled, 0.0))
    b_q_right_hi = tl.trans(tl.where(~m_lo[:, None], b_q_scaled, 0.0))
    b_k_right_lo = tl.trans(tl.where(m_lo[:, None], b_k_scaled, 0.0))
    b_k_right_hi = tl.trans(tl.where(~m_lo[:, None], b_k_scaled, 0.0))

    b_Aqk_t = tl.dot(b_k_left_lo, b_q_right_lo, out_dtype=tl.float32)
    b_Aqk_t += tl.dot(b_k_left_lo, b_q_right_hi, out_dtype=tl.float32)
    b_Aqk_t += tl.dot(b_k_left_hi, b_q_right_hi, out_dtype=tl.float32)
    b_Akk_t = tl.dot(b_k_left_lo, b_k_right_lo, out_dtype=tl.float32)
    b_Akk_t += tl.dot(b_k_left_lo, b_k_right_hi, out_dtype=tl.float32)
    b_Akk_t += tl.dot(b_k_left_hi, b_k_right_hi, out_dtype=tl.float32)
    b_Aqk = tl.trans(b_Aqk_t)
    b_Akk = tl.trans(b_Akk_t)
    b_Aqk = b_Aqk * b_q_rstd[:, None] * b_k_rstd[None, :]
    b_Akk = b_Akk * b_k_rstd[:, None] * b_k_rstd[None, :]
    ...
    tl.store(p_g_last, b_gn.to(g_last.dtype.element_ty), boundary_check=(0, 1))
    b_kg_val = b_k3 * tl.where(m_c[:, None], exp2(b_gn - b_gc), 0.0)
    p_kg = tl.make_block_ptr(
        ws,
        (T, 3 * K),
        (HV * 3 * K, 1),
    ...
    # (I-L)(I+L^2)(I+L^4)(I+L^8)(I+L^16) = I for a strictly
    # lower-triangular 32x32 L.
    b_L = b_Akk.to(tl.float16)
    b_Ai = m_I.to(tl.float16) - b_L
    b_L2 = tl.dot(b_L, b_L, out_dtype=tl.float16)
    b_Ai = b_Ai + tl.dot(b_Ai, b_L2, out_dtype=tl.float16)
    ...
    b_L16 = tl.dot(b_L8, b_L8, out_dtype=tl.float16)
    b_Ai = b_Ai + tl.dot(b_Ai, b_L16, out_dtype=tl.float16)
    p_Akk = tl.make_block_ptr(Akk, (BT, BT), (BT, 1), (0, 0), (BT, BT), (1, 0))
    tl.store(p_Akk, b_Ai.to(Akk.dtype.element_ty))
...
@triton.jit(do_not_specialize=["T"])
def _kda_fwd_state_output_core_triton_kernel(
    v,
    beta,
    gk,
    Aqk,
    ...
    """TLE-free K2; one S60 program owns a complete 128x128 head state."""
    i_pid = tl.program_id(0)
    n_v_tiles = tl.cdiv(V, BV)
    i_v = i_pid % n_v_tiles
    i_nh = i_pid // n_v_tiles
...
def _kda_fwd_intra_core_triton(
    q,
    k,
    g,
    beta,
    ...
    _kda_fwd_intra_core_triton_kernel[grid](
        q=q,
        k=k,
        g=g,
        beta=beta,
...
def _kda_fwd_state_output_core_triton(
    v,
    beta,
    Akk,
    gk,
    ...
    BV = 128
    _kda_fwd_state_output_core_triton_kernel[
        (triton.cdiv(V, BV) * N * HV,)
    ](
        v=v,
        beta=beta,
...
def chunk_kda_fwd_infer_triton(
    ...
    **kwargs,
    ...
    if BT == 32 and core_reason is None:
        ws, Aqk, Akk, g_last = _kda_fwd_intra_core_triton(
            q=q,
            k=k,
            g=g,
            beta=beta,
        ...
        return _kda_fwd_state_output_core_triton(
            v=v,
            beta=beta,
            Akk=Akk,
            gk=g_last,
...
def _validate_chunk_kda_inputs(
    ...
    if q.device.type != "gcu":
        raise ValueError(f"chunk_kda requires Enflame GCU inputs, got {q.device}")
    if chunk_size not in (16, 32):
        raise ValueError(
            f"chunk_kda TLE path requires chunk_size in {{16, 32}}, "
            f"got {chunk_size}"
        )
    ...
    if cu_seqlens is not None:
        ...
        if cu_seqlens.dtype != torch.int32:
            raise ValueError(
                "Enflame KDA requires cu_seqlens with dtype torch.int32; "
                "GCU300 kernels do not support 64-bit index data"
            )
...
# =============================================================================
# Public entry: validate, then dispatch
# =============================================================================
...
def chunk_kda(
    ...
    **kwargs,
    ...
    can be forced with ``FLAG_ATTN_CHUNK_KDA_BACKEND``. If ``chunk_size`` is
    omitted, strict-compatible S60 core paths use BT=32 while wider fallback
    paths retain the legacy BT=16 default. Explicit BT=16 or BT=32 requests
    are honored.
    """
    A_log = kwargs.get("A_log")
    dt_bias = kwargs.get("dt_bias")
    fallback_chunk_size = 16 if chunk_size is None else chunk_size
    strict_chunk_size = 32 if chunk_size is None else chunk_size
    ...
    strict_tle_reason = strict_tle_input_error(
        ...
        A_log=A_log,
        dt_bias=dt_bias,
        chunk_size=strict_chunk_size,
    )
    triton_core_reason = strict_tle_input_error(
        q=q,
        k=k,
        v=v,
        g=g,
    ...
    generic_tle_chunk_size = (
        strict_chunk_size if strict_tle_reason is None else fallback_chunk_size
    )
    triton_chunk_size = (
        strict_chunk_size if triton_core_reason is None else fallback_chunk_size
    )
    ...
    if backend in {"auto", "tle"} and HAS_TLE_KDA:
        return chunk_kda_fwd_infer(
            ...
            state_v_first=state_v_first,
            cu_seqlens=cu_seqlens,
            chunk_size=generic_tle_chunk_size,
            safe_gate=safe_gate,
            lower_bound=lower_bound,
            A_log=A_log,
            dt_bias=dt_bias,
