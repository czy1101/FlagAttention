...
INTRA_TLE_MAXNREG_CANDIDATES = (64, 72, 96)
INTRA_TLE_SMEM_REUSE_MIN_K = 256


def _require_contiguous(name: str, tensor: torch.Tensor) -> None:
    if not tensor.is_contiguous():
        raise NotImplementedError(f"MetaX chunk_gdn2 requires contiguous {name}")
...
# TLE kernels for GDN2 prefill with BT=16.
if HAS_TLE_GDN2:

    # =============================================================================
    # Fused intra-chunk kernel, parallel over (chunk, batch*head).
    # =============================================================================
    ...
    @triton.heuristics(
        {
            ...
            "USE_INTRA_SMEM_REUSE": lambda args: args["K"]
            >= INTRA_TLE_SMEM_REUSE_MIN_K,
        }
    )
    @triton.autotune(
        configs=[
    ...
    def _chunk_gdn2_fwd_intra_infer_kernel(
        ...
        USE_INTRA_SMEM_REUSE: tl.constexpr,
    ):
        i_t = tl.program_id(0)
        ...
        if USE_INTRA_SMEM_REUSE:
            # MetaX C550:
            # Avoid creating [BT, BK] local_ptr subviews from a [BT, K]
            # TLE shared-memory allocation.  The MetaX lowering currently
            # generates an invalid cross-address-space llvm.bitcast for that
            # pattern.  Instead, reuse one native [BT, BK] shared tile.
            q_buf = tle.gpu.alloc(
                [BT, BK],
            ...
            b_buf = tle.gpu.alloc(
                [BT, BK],
                dtype=b.dtype.element_ty,
                scope=tle.gpu.smem,
                nv_mma_shared_layout=False,
            )

            smem_rows = tl.broadcast_to(tl.arange(0, BT)[:, None], (BT, BK))
            smem_cols = tl.broadcast_to(tl.arange(0, BK)[None, :], (BT, BK))

            q_sp = tle.gpu.local_ptr(q_buf, (smem_rows, smem_cols))
            k_sp = tle.gpu.local_ptr(k_buf, (smem_rows, smem_cols))
            g_sp = tle.gpu.local_ptr(g_buf, (smem_rows, smem_cols))
            b_sp = tle.gpu.local_ptr(b_buf, (smem_rows, smem_cols))
        ...
        if USE_QK_L2NORM:
            ...
            for i_k in range(tl.cdiv(K, BK)):
                if USE_INTRA_SMEM_REUSE:
                    ...
                    b_q_blk = tl.load(q_sp).to(tl.float32)
                    b_k_blk = tl.load(k_sp).to(tl.float32)
                else:
                    p_q = tl.make_block_ptr(
        ...
        for i_k in range(tl.cdiv(K, BK)):
            if USE_INTRA_SMEM_REUSE:
                ...
                b_q_blk = tl.load(q_sp).to(tl.float32) * b_q_rstd[:, None]
                b_k_blk = tl.load(k_sp).to(tl.float32) * b_k_rstd[:, None]
                b_g_blk = tl.load(g_sp)
                b_erase_blk = tl.load(b_sp).to(tl.float32)
            else:
                p_q = tl.make_block_ptr(
                    q, (T, K), (H * K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0)
                )
                ...
                b_k_blk = (
                    tl.load(p_k, boundary_check=(0, 1)).to(tl.float32)
                    * b_k_rstd[:, None]
                )
        ...
        # Phase 2: 16x16 Neumann-series inverse.
        # MetaX C550: FP16 MMA inputs are supported, but use FP32 accumulator/output.
        b_L = b_Akk.to(tl.float16)
        b_Ai = m_I.to(tl.float32) - b_L.to(tl.float32)
    ...
    # =============================================================================
    # Fused state propagation and output, sequential over chunks per
    # (batch, head, V-tile).
    # =============================================================================
    ...
    def chunk_gdn2_fwd_h_o_infer(
        ...
        if output_final_state:
            if state_v_first:
                final_state = kg.new_empty(N, H, V, K, dtype=torch.float32)
            else:
    ...
    def chunk_gdn2_fwd_infer(
        ...
        **kwargs,
        ...
        """Run the fused inference forward path for GDN2.
    ...
    *,
    ...
    if not HAS_TLE_GDN2:
        ...
        if use_qk_l2norm_in_kernel:
            raise NotImplementedError(
                "MetaX chunk_gdn2 requires TLE for in-kernel Q/K L2 normalization."
            )
        (
            o,
            final_state,
