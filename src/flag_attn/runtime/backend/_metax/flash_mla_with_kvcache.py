...
# C550 exposes a 64 KiB shared-memory limit per CTA, so the original
# BH=64/BK=64 warp-specialized tile is too large even after removing pipes.
METAX_SERIAL_TLE_BH = 32
METAX_SERIAL_TLE_BK = 16


# ============================================================================
# METAX_GLOBAL_SPLIT_AUTO_DISPATCH_V2
#
# Dense SQ=1 MetaX global split policy.
#
# Explicit num_splits always wins.
#
# Auto mode:
#   1. build a cheap host-visible cache key;
#   2. cache hit  -> reuse split, zero D2H;
#   3. cache miss -> copy cache_seqlens to CPU once, choose split, cache it.
#
# The cache key includes:
#   - input geometry;
#   - cache_seqlens data_ptr;
#   - PyTorch tensor version counter when available.
#
# Therefore normal PyTorch in-place updates such as fill_ / add_ invalidate
# the cached decision without requiring a device synchronization.
# ============================================================================


def _metax_choose_global_split_v1(
    lengths,
    batch_size,
    seq_q,
    num_heads_q,
    ...
    """
    Global Split Policy V1.

    `lengths` is a host-side Python list[int].
    """

    if not lengths:
        return 1

    bh = METAX_SERIAL_TLE_BH
    ...
    # --------------------------------------------------------
    # Policy calibrated from controlled C550 benchmarks.
    # --------------------------------------------------------
...
def _metax_cache_seqlens_version(cache_seqlens):
    """
    Host-only invalidation signal.

    PyTorch increments Tensor._version for normal in-place mutations.
    Do not fail if a backend tensor implementation does not expose it.
    """

    try:
        return int(cache_seqlens._version)
    except Exception:
        return None


def _metax_make_auto_split_cache_key(
    q,
    k_cache,
    cache_seqlens,
    head_dim_v,
    ...
    return (
        ...
        int(k_cache.shape[1]), # PAGE_SIZE
        int(k_cache.shape[2]), # HKV

        int(cache_seqlens.data_ptr()),

        _metax_cache_seqlens_version(
            cache_seqlens
        ),
    )


def _metax_auto_global_split_v2(
    sched_meta,
    q,
    k_cache,
    cache_seqlens,
    ...
    """
    Choose or reuse the cached global split.

    Cache hit:
        no GPU->CPU synchronization.

    Cache miss:
        one tiny D2H copy of cache_seqlens.
    """

    key = _metax_make_auto_split_cache_key(
        q,
        k_cache,
        cache_seqlens,
        head_dim_v,
    ...
    old_key = getattr(
        sched_meta,
        "_metax_auto_split_cache_key",
        None,
    )

    cached_split = getattr(
        sched_meta,
        "_metax_global_num_splits",
        None,
    )

    if (
        old_key == key
        and cached_split is not None
    ):
        return int(cached_split)

    # --------------------------------------------------------
    # Cache miss:
    # exactly one B-element int32 D2H transfer.
    # --------------------------------------------------------

    if cache_seqlens.device.type == "cpu":
        lengths = [
            int(x)
            for x in cache_seqlens.tolist()
        ]
    else:
        lengths = [
            int(x)
    ...
    split = _metax_choose_global_split_v1(
        lengths,
    ...
    sched_meta._metax_auto_split_cache_key = key
    sched_meta._metax_global_num_splits = split

    # Debug / validation only.
    sched_meta._metax_auto_split_recompute_count = (
        getattr(
            sched_meta,
            "_metax_auto_split_recompute_count",
            0,
        )
        + 1
    )
...
#
# Keep the metadata interface compatible with FlashMLA callers.
# ============================================================================
...
def get_mla_metadata(*args, **kwargs) -> Tuple[FlashMLASchedMeta, None]:
    """Return an empty scheduler metadata object."""
...
@triton.autotune(
    configs=[
        ...
        # MetaX C550-friendly configs.
        # C550 exposes a 64 KiB shared-memory limit per CTA.
...
@triton.autotune(
    configs=[
        ...
        # MetaX C550-friendly configs.
        # BK must remain >= 16 because it is also the K dimension
        # of several tl.dot operations.
        triton.Config({"BK": 32, "BH": 16}, num_warps=4, num_stages=1),
        triton.Config({"BK": 16, "BH": 32}, num_warps=4, num_stages=1),
        triton.Config({"BK": 16, "BH": 16}, num_warps=4, num_stages=1),
    ],
    key=[
        "HQ",
...
def _sparse_decode_model1_kernel(
    ...
    for ck in range(NK):
        ...
        # Use the token-id bound instead of `block_ids < NUM_BLOCKS`.
        # They are mathematically equivalent for non-negative kv_ids, but the
        # latter makes FlagTree AABS incorrectly associate BK with NUM_BLOCKS
        # and shrink BK below tl.dot's minimum K dimension.
        valid_ids = (
            t_msk
            & (kv_ids >= 0)
            & (kv_ids < NUM_BLOCKS * PAGE_SIZE)
        )
    ...
    if HAVE_EXTRA:
        ...
        for ck in range(ENK):
            ...
            valid_ids = (
                t_msk
                & (kv_ids >= 0)
                & (kv_ids < EXTRA_NUM_BLOCKS * EXTRA_PAGE_SIZE)
            )
            block_ids = tl.where(valid_ids, block_ids, 0)
            rel_ids = tl.where(valid_ids, rel_ids, 0)
...
@triton.autotune(
    configs=[
        ...
        # MetaX C550-friendly configs: 64 KiB shared-memory limit
...
def _dense_decode_kernel(
    ...
    HAVE_PE: tl.constexpr = DQK > HEAD_DIM_V
    if HAVE_PE:
        PE_DIM: tl.constexpr = DQK - HEAD_DIM_V
        offs_pe = tl.arange(0, PE_DIM)
        offs_d_pe = HEAD_DIM_V + offs_pe
        offs_q_pe = (
            i_b * stride_q_b
            + i_sq * stride_q_sq
            + cur_head[:, None] * stride_q_h
    ...
    for i in range(0, loop_time):
        # KV cache may exceed the signed int32 element-offset range.
        # Promote page/token address arithmetic to int64 before multiplying
        # by stride_kv_bs.
        kv_page_number = tl.load(
            Block_table + offs_n // PAGE_SIZE
        ).to(tl.int64)
        kv_loc = (
            kv_page_number * PAGE_SIZE
            + (offs_n % PAGE_SIZE).to(tl.int64)
        )
        ...
        # Add RoPE contribution only when DQK > HEAD_DIM_V.
        if HAVE_PE:
            offs_k_pe = (
...
def flash_mla_with_kvcache(
    ...
    # METAX_GLOBAL_SPLIT_AUTO_DISPATCH_V2
    #
    # Explicit caller-provided num_splits has highest priority.
    #
    # num_splits=None activates the automatic global split policy only for
    # the validated MetaX dense SQ=1 fast path.
    # ========================================================================

    if num_splits is None:

        _metax_auto_split_supported = (
            indices is None
            and q.shape[1] == 1
            and q.shape[2] % METAX_SERIAL_TLE_BH == 0
            and q.shape[-1] in (512, 576)
            and head_dim_v == 512
            and k_cache.shape[1] == 64
            and k_cache.shape[2] == 1
            and cache_seqlens is not None
            and torch.is_tensor(cache_seqlens)
        )

        if _metax_auto_split_supported:

            num_splits = (
                _metax_auto_global_split_v2(
                    sched_meta,
                    q,
                    k_cache,
                    cache_seqlens,
    ...
    # METAX_KV_WORKLOAD_SCHEDULER_SPLIT_V1
...
def _sparse_decode_dispatch(
    ...
    # The sparse kernels autotune BH, so the launch grid must use
    # the selected META["BH"].  A fixed BH=64 grid would under-launch
    # when C550 selects BH=16 or BH=32.
    grid = lambda META: (
        batch_size
        * seq_q
        * triton.cdiv(num_heads_q, META["BH"]),
    )
...
def _dense_decode_dispatch(
    ...
    # METAX_KV_WORKLOAD_SCHEDULER_SPLIT_V1
    #
    # V1 intentionally has NO automatic policy.
    #
    # num_splits=None / 1:
    #     existing serial TLE path
    #
    # num_splits=2/4/8/16:
    #     forced split-KV path
    #
    # We first measure the crossover, then build the scheduler.
    # ========================================================
    ...
    if num_splits is not None and num_splits > 1:
        try:
            is_metax = (
                "metax"
                in torch.cuda.get_device_name(q.device).lower()
            )
        except Exception:
            is_metax = False
        ...
        supported = (
            ...
            and num_heads_q % METAX_SERIAL_TLE_BH == 0
        )

        if not supported:
            raise NotImplementedError(
                "scheduler split V1 currently requires "
                "MetaX, SQ=1, DQK=512/576, DV=512, "
                "PAGE_SIZE=64, HQ divisible by 32"
            )

        _metax_dense_decode_split_k_launch(
            q,
            kv_flat,
            block_table,
            cache_seqlens,
...
# METAX_KV_WORKLOAD_SCHEDULER_SPLIT_V1
#
# Dense MLA split-KV execution engine.
#
# This is NOT the automatic scheduler yet.
# num_splits is explicitly forced to 2 / 4 / 8 for crossover measurement.
# ============================================================================


@triton.jit
def _metax_dense_decode_split_k_partial(
    q,
    stride_qb,
    stride_qsq,
    stride_qh,
    ...
    TD: tl.constexpr = DQK - D
    HAVE_TAIL: tl.constexpr = TD > 0
    TDP: tl.constexpr = 64 if TD > 0 else 1

    offs_h = tl.arange(0, BH)
    offs_d = tl.arange(0, DPH)
    ...
    # --------------------------------------------------------
    # Split by LOGICAL PAGE.
    #
    # Each split owns a contiguous logical-page range.
    # This preserves the page-hoist idea.
    # --------------------------------------------------------

    cache_seqlen = tl.load(
        cache_seqlens + i_b
    )
    ...
    pages_per_split = tl.cdiv(
        num_pages,
        NUM_SPLITS,
    )

    page_begin = (
        split_id * pages_per_split
    )

    page_end = tl.minimum(
        page_begin + pages_per_split,
        num_pages,
    )
    ...
    for logical_page in tl.range(
        ...
        page_token_base = (
            phys_page.to(tl.int64)
            * PAGE_SIZE
        )

        # PAGE_SIZE=64 / BK=16:
        # four compute tiles per logical page.
        for in_page_start in tl.range(
            0,
            PAGE_SIZE,
            BK,
        ):
    ...
    # --------------------------------------------------------
    # Store normalized partial O and its partial LSE.
    #
    # O_s = sum(exp(score - m_s) V) / l_s
    # LSE_s = m_s + log(l_s)
    #
    # Merge kernel can reconstruct global normalization.
    # --------------------------------------------------------
...
@triton.jit
def _metax_dense_decode_split_k_merge(
    partial_out,
    partial_lse,
    ...
    for split_id in tl.static_range(
        0,
        NUM_SPLITS,
    ):
        row = (
    ...
    tl.store(
        out_base + offs_d[None, :],
        (
            acc_l
            * inv_denom[:, None]
        ).to(
            output.dtype.element_ty
        ),
        mask=mask_h[:, None],
    )
...
# METAX_SPLIT_WORKSPACE_CACHE_V1
#
# Scheduler benchmark / production-style workspace reuse.
# Keyed by the exact required tensor geometry.
_METAX_SPLIT_WORKSPACE_CACHE = {}


def _get_metax_split_workspace(
    q,
    num_splits,
    bsq,
    num_heads_q,
    ...
    ws = _METAX_SPLIT_WORKSPACE_CACHE.get(key)
    ...
    if ws is None:
        ...
        _METAX_SPLIT_WORKSPACE_CACHE[key] = ws

    return ws


def _metax_dense_decode_split_k_launch(
    q,
    kv_flat,
    block_table,
    cache_seqlens,
    ...
    # Workspace
    #
    # V1: simple temporary allocation.
    #
    # Once the scheduler policy is finalized, workspace should
    # be cached / managed by scheduler metadata rather than
    # treated as the final production design.
    # --------------------------------------------------------

    partial_out, partial_lse = _get_metax_split_workspace(
        q,
        num_splits,
        bsq,
        num_heads_q,
    ...
    _metax_dense_decode_split_k_partial[
        partial_grid
    ](
        q,
        q.stride(0),
    ...
    _metax_dense_decode_split_k_merge[
        merge_grid
    ](
        partial_out,
        partial_lse,
...
def _tle_dense_decode_launch(
    ...
    """Launch TLE dense decode kernel."""

    # MetaX C550 currently lacks the TLE pipe IR builder
    # (create_pipe_create).  Keep the original warp-specialized path for
    # backends that support it, but use a serial TLE path on MetaX.
    if "metax" in torch.cuda.get_device_name(q.device).lower():
        BH = METAX_SERIAL_TLE_BH
        BK = METAX_SERIAL_TLE_BK

        assert num_heads_q % BH == 0
        ...
        block_table_flat = block_table.reshape(
            batch_size * seq_q, -1
        ).contiguous()

        cache_seqlens_flat = cache_seqlens.reshape(
            batch_size * seq_q
        ).contiguous()

        grid = (batch_size * seq_q * RH,)

        _tle_dense_decode_serial_fwd[grid](
            q_flat,
            kv_flat,
            block_table_flat,
            cache_seqlens_flat,
...
if HAS_TLE:
    ...
    @triton.jit
    def _tle_dense_decode_serial_fwd(
        q,
        kv_cache,
        block_table,
        cache_seqlens,
        ...
        """
        MetaX serial TLE dense decode.

        Keep TLE shared-memory primitives, but remove tle.pipe() and
        warp-specialized producer/consumer synchronization.

        One program computes one BH-head block.
        KV is processed BK tokens at a time with online softmax.
        """
        ...
        # ----------------------------------------------------
        # Q staging in TLE shared memory
        #
        # BH=16:
        #   left  = 16*256*2 = 8 KiB
        #   right = 16*256*2 = 8 KiB
        #   tail  = 16*64*2  = 2 KiB
        #
        # Far below C550's 64 KiB CTA shared-memory limit.
        # ----------------------------------------------------
        ...
        rows_q = tl.broadcast_to(
            offs_h[:, None],
            (BH, DPH),
        )

        cols_q = tl.broadcast_to(
            offs_d[None, :],
            (BH, DPH),
        )
        ...
        q_l_global = tl.load(q_l_ptr)
        q_r_global = tl.load(q_r_ptr)

        q_l_smem_ptr = tle.gpu.local_ptr(
            sQ_l,
            (rows_q, cols_q),
        )

        q_r_smem_ptr = tle.gpu.local_ptr(
            sQ_r,
            (rows_q, cols_q),
        )

        tl.store(q_l_smem_ptr, q_l_global)
        tl.store(q_r_smem_ptr, q_r_global)

        q_l = tl.load(q_l_smem_ptr)
        q_r = tl.load(q_r_smem_ptr)

        if HAVE_TAIL:
            offs_td = tl.arange(0, TDP)

            rows_qt = tl.broadcast_to(
                offs_h[:, None],
                (BH, TDP),
            )

            cols_qt = tl.broadcast_to(
                offs_td[None, :],
                (BH, TDP),
            )
            ...
            q_tail_global = tl.load(
                q_tail_ptr,
                mask=(offs_td < TD)[None, :],
                other=0.0,
            )

            q_tail_smem_ptr = tle.gpu.local_ptr(
                sQ_tail,
                (rows_qt, cols_qt),
            )
            ...
            q_tail = tl.load(q_tail_smem_ptr)
        ...
        #
        #   baseline:
        #       every BK tile -> load block_table
        #
        #   page-hoisted:
        #       one physical-page lookup
        #       -> four BK16 subtiles
        #
        # Keep the exact same QK / online-softmax / PV computation and
        # validity masks.  No prefetch, no extra shared buffers.
        # ============================================================

        num_pages = tl.cdiv(cache_seqlen, PAGE_SIZE)

        for logical_page in tl.range(num_pages):

            # One block-table lookup per 64-token page.
            phys_page = tl.load(
                block_table_ptr + logical_page
            )

            page_token_base = (
                phys_page.to(tl.int64) * PAGE_SIZE
            )

            # PAGE_SIZE=64, BK=16 -> four subtiles per physical page.
            for in_page_start in tl.range(
                0,
                PAGE_SIZE,
                BK,
                ...
                # MLA dense V == first 512 K dimensions.
                acc_l = tl.dot(
                    p_bf16,
                    k_l,
                    acc=acc_l,
        ...
        out_l = acc_l * inv_sum[:, None]
        out_r = acc_r * inv_sum[:, None]

        out_base = output + q_row * D
        ...
        tl.store(
            out_l_ptr,
            out_l.to(output.dtype.element_ty),
        )

        tl.store(
            out_r_ptr,
            out_r.to(output.dtype.element_ty),
        )

        lse_value = e_max + tl.log(e_sum)

        tl.store(
            lse + i_sq64 * G + h_base + offs_h,
            lse_value,
        )
