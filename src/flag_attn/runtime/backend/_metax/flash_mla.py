...
@triton.jit
def flash_mla_splitkv_tle_metax_serial_kernel(
    Q,
    Kv,
    Block_table,
    B_seq_len,
    ...
    D_CHUNK: tl.constexpr,
    META_FIELDS: tl.constexpr,
    SERIAL_M: tl.constexpr,
    SERIAL_N: tl.constexpr,
):
    """
    MetaX TLE serial FlashMLA path.

    Keep:
      - TLE shared-memory allocation
      - TLE local pointers
      - split-KV scheduler
      - online softmax
      - partial O/LSE + combine

    Remove from the MetaX execution path:
      - tle.pipe
      - producer/consumer pipe synchronization
      - tle.gpu.warp_specialize

    BLOCK_M/BLOCK_N remain scheduler-level tiles.
    SERIAL_M/SERIAL_N are smaller execution tiles used to fit C550
    ...
    # TLE shared memory
    #
    # scheduler tile remains 64x64, but only one
    # SERIAL_M x SERIAL_N tile lives in shared memory at once.
    # ========================================================
    ...
    offs_m = tl.arange(0, SERIAL_M)
    offs_n = tl.arange(0, SERIAL_N)
    offs_half = tl.arange(0, HALF_DIM_V)

    q_rows = tl.broadcast_to(
        offs_m[:, None],
        (SERIAL_M, HALF_DIM_V),
    )
    q_cols = tl.broadcast_to(
        offs_half[None, :],
        (SERIAL_M, HALF_DIM_V),
    )

    k_rows = tl.broadcast_to(
        offs_n[:, None],
        (SERIAL_N, HALF_DIM_V),
    )
    k_cols = tl.broadcast_to(
        offs_half[None, :],
        (SERIAL_N, HALF_DIM_V),
    )

    if HAVE_TAIL:
        offs_tail = tl.arange(0, D_CHUNK)

        q_rows_tail = tl.broadcast_to(
            offs_m[:, None],
            (SERIAL_M, D_CHUNK),
        )
        q_cols_tail = tl.broadcast_to(
            offs_tail[None, :],
            (SERIAL_M, D_CHUNK),
        )

        k_rows_tail = tl.broadcast_to(
            offs_n[:, None],
            (SERIAL_N, D_CHUNK),
        )
        k_cols_tail = tl.broadcast_to(
            offs_tail[None, :],
            (SERIAL_N, D_CHUNK),
        )

    # ========================================================
    # Requests assigned to this scheduler partition
    # ========================================================

    for batch_idx in tl.range(
        begin_req_idx,
        end_req_idx + 1,
    ):
        seq_len = tl.load(B_seq_len + batch_idx)

        start_block_idx = tl.where(
            batch_idx == begin_req_idx,
            begin_block_idx_meta,
            0,
        )

        full_end_block_idx = tl.cdiv(
            seq_len,
            BLOCK_N,
        )

        end_block_idx = tl.where(
            batch_idx == end_req_idx,
            end_block_idx_meta,
            full_end_block_idx,
        )

        n_split_idx = tl.where(
            batch_idx == begin_req_idx,
            begin_split_idx,
            0,
        )
        ...
        split_idx = (
            tl.load(Num_splits + batch_idx)
            + n_split_idx
        )

        block_table_base = (
            batch_idx * stride_block_table_b
        )

        # ====================================================
        # Original BLOCK_M head tile -> SERIAL_M subtiles
        # ====================================================

        for m_offset in tl.range(
            0,
            BLOCK_M,
            SERIAL_M,
        ):
            ...
            #   [B * S_Q * H_Q, HEAD_DIM]
            #
            # FlashMLA decode test has S_Q=1, matching original
            # output_row = batch_idx * head_num + head_base logic.
            ...
            # =================================================
            # Standard single-stream online-softmax state
            # =================================================
            ...
            for block_idx in tl.range(
                ...
                kv_page_row = page.to(tl.int64) * PAGE_SIZE

                # =============================================
                # BLOCK_N -> SERIAL_N token subtiles
                # =============================================

                for n_offset in tl.range(
                    0,
                    BLOCK_N,
                    SERIAL_N,
                ):
                    ...
                    valid_n = logical_n < seq_len

                    kv_rows_global = (
                        kv_page_row.to(tl.int64)
                        + n_offset
                        + offs_n.to(tl.int64)
                    )

                    # =========================================
                    # Current KV subtile -> TLE shared memory
                    #
                    # First HEAD_DIM_V dimensions are both
                    # attention content-K and compressed V.
                    # Tail dimensions only participate in QK.
                    # =========================================
                    ...
                    # Replaces producer commit / consumer wait.
                    tl.debug_barrier()
                    ...
                    # Online softmax
                    #
                    # No sM/sL pipe exchange is required now,
                    # because all KV blocks are consumed by one
                    # serial softmax state.
                    # =========================================
                    ...
                    # PV
                    #
                    # Current compressed KV content dimensions
                    # are reused as V, exactly as the original
                    # two consumers do for K_l/K_r.
                    # =========================================
                    ...
                    # Current shared KV tile must be fully
                    # consumed before the next loop overwrites it.
                    tl.debug_barrier()

            # =================================================
            # Normalize this split
            # =================================================
            ...
            lse_vals = tl.where(
                valid,
                tl.log(safe_sum) + e_max,
                float("-inf"),
            )

            # =================================================
            # Unsplit request -> final O
            # =================================================
            ...
            if is_no_split:
                ...
                tl.store(
                    ...
                    * stride_o_h
                    + HALF_DIM_V
                    + offs_half[None, :],
                    out_r.to(O.dtype.element_ty),
                    mask=mask_h[:, None],
                )

            # =================================================
            # Split request -> partial O + LSE
            # =================================================
                ...
                tl.store(
                    ...
                    * stride_oaccum_h
                    + offs_half[None, :],
                    out_l.to(O_accum.dtype.element_ty),
                    mask=mask_h[:, None],
                )
                ...
                tl.store(
                    ...
                    * stride_oaccum_h
                    + HALF_DIM_V
                    + offs_half[None, :],
                    out_r.to(O_accum.dtype.element_ty),
                    mask=mask_h[:, None],
                )
    ...
    def run(
        ...
        *,
        ...
        if vendor_name == "metax":
            flash_mla_splitkv_tle_metax_serial_kernel[
                (self.num_m_blocks, self.num_sm_parts)
            ](
                q_flat,
                kv_flat,
                block_table_tle,
            ...
            flash_mla_splitkv_ws_tle_kernel[
                (self.num_m_blocks, self.num_sm_parts)
            ](
                q_desc,
                q_tail_desc,
...
def _try_flash_mla_tle(
    ...
    # MetaX Triton backend currently does not provide the NVIDIA-specific
    # nv_mma_shared_layout IR builder required by the TLE FlashMLA kernel.
    # Fall back to the plain Triton implementation on MetaX.
...
def flash_mla(
    ...
    if vendor_name == "metax":
        # C550 exposes 64 KiB shared memory per block. The generic
        # major-8 configuration (BLOCK_H=32, num_stages=2) requires
        # about 148 KiB for this kernel, so use a smaller head tile
        # and a single pipeline stage.
        BLOCK_H = 16
        num_stages = 1
    elif major == 9:
        BLOCK_H = 64
        ...
        raise RuntimeError(f"flash_mla is unsupported on device {q.device}")
    BLOCK_N = 16 if vendor_name == "metax" else 64
    grid = (
    ...
    with torch.cuda.device(q.device):
        flash_mla_attn_kernel[grid](
            ...
            HEAD_DIM_V=dv,
            HEAD_DIM=d,
            num_warps=4 if vendor_name == "metax" else 8,
