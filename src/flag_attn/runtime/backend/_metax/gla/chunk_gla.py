...
FWD_A_NUM_WARPS = [1] if HAS_TLE else [1, 2, 4, 8]
BWD_DV_NUM_WARPS = [1] if HAS_TLE else [2, 4, 8]
NUM_STAGES_234 = [2] if HAS_TLE else [2, 3, 4]
NUM_STAGES_23 = [2] if HAS_TLE else [2, 3]
...
@triton.autotune(
    configs=[
        ...
        for BK in [32, 64]
        for num_warps in FWD_A_NUM_WARPS
        for num_stages in NUM_STAGES_234
    ],
    key=["BC"],
)
...
@triton.autotune(
    configs=[
        ...
        for num_warps in FWD_A_NUM_WARPS
        for num_stages in NUM_STAGES_23
    ],
    key=["BK", "BT"],
)
@triton.jit(do_not_specialize=["T"])
...
@triton.autotune(
    configs=[
        ...
        for BK in BK_LIST
        for BV in BV_LIST
        for num_warps in BWD_DV_NUM_WARPS
        for num_stages in NUM_STAGES_234
    ],
