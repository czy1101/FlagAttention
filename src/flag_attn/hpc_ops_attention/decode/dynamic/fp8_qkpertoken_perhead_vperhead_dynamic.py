...
def _compute_mtp2___finalize_owned_heads(
    SPLIT_OUT, LSE, OUT, FINAL_WEIGHT_SMEM,
    batch, hkv, first_h_in_group, n_chunks,
    H_Q: tl.constexpr, HEADS_PER_GROUP: tl.constexpr, DV: tl.constexpr,
    SO_STRIDE_B: tl.constexpr, SO_STRIDE_C: tl.constexpr,
    SO_STRIDE_M: tl.constexpr, SO_STRIDE_H: tl.constexpr,
    LSE_STRIDE_B: tl.constexpr, LSE_STRIDE_C: tl.constexpr,
    LSE_STRIDE_HKV: tl.constexpr, LSE_STRIDE_M: tl.constexpr,
    LSE_STRIDE_HG: tl.constexpr,
    O_STRIDE_B: tl.constexpr, O_STRIDE_M: tl.constexpr,
    O_STRIDE_H: tl.constexpr, MAX_FINAL_CHUNKS: tl.constexpr,
    OWNED_HEADS: tl.constexpr, HEAD_STRIDE: tl.constexpr,
):
    offs_c = tl.arange(0, MAX_FINAL_CHUNKS)
    ...
    lse_values = tl.load(
        LSE + batch * LSE_STRIDE_B + offs_c[:, None, None] * LSE_STRIDE_C
        + hkv * LSE_STRIDE_HKV + offs_m[None, None, :] * LSE_STRIDE_M
    ...
    for chunk in tl.static_range(0, MAX_FINAL_CHUNKS):
        ...
        chunk_weight = tl.load(
            ...
            mask=chunk_valid, other=0.0,
        )
        partial = tl.load(
            SPLIT_OUT + batch * SO_STRIDE_B + chunk * SO_STRIDE_C
            + offs_m[None, None, :] * SO_STRIDE_M
            + hq[None, :, None] * SO_STRIDE_H + offs_v[:, None, None],
            mask=chunk_valid[None, :, :], other=0.0,
        )
        acc += partial * chunk_weight[None, :, :]
    tl.debug_barrier()
    tl.store(
        OUT + batch * O_STRIDE_B + offs_m[None, None, :] * O_STRIDE_M
        + hq[None, :, None] * O_STRIDE_H + offs_v[:, None, None],
        acc,
        mask=valid_head[None, :, None] & (denom[None, :, :] > 0.0),
    )

@triton.jit
def _compute_mtp2__fp8_kvpertensor_decode_mtp2_final_kernel(Q, K_DESC, KS_DESC, VT_DESC, BLOCK_IDS, TASK_MAP, QSCALE, KSCALE, VSCALE, SPLIT_OUT, LSE, COMPLETION, OUT, mesh: tl.constexpr, B: tl.constexpr, H_Q: tl.constexpr, HEADS_PER_GROUP: tl.constexpr, D: tl.constexpr, DV: tl.constexpr, BLOCK_SIZE: tl.constexpr, MAX_BLOCKS: tl.constexpr, BLOCK_N: tl.constexpr, Q_STRIDE_B: tl.constexpr, Q_STRIDE_M: tl.constexpr, Q_STRIDE_H: tl.constexpr, QS_STRIDE_B: tl.constexpr, QS_STRIDE_M: tl.constexpr, QS_STRIDE_H: tl.constexpr, SO_STRIDE_B: tl.constexpr, SO_STRIDE_C: tl.constexpr, SO_STRIDE_M: tl.constexpr, SO_STRIDE_H: tl.constexpr, LSE_STRIDE_B: tl.constexpr, LSE_STRIDE_C: tl.constexpr, LSE_STRIDE_HKV: tl.constexpr, LSE_STRIDE_M: tl.constexpr, LSE_STRIDE_HG: tl.constexpr, O_STRIDE_B: tl.constexpr, O_STRIDE_M: tl.constexpr, O_STRIDE_H: tl.constexpr, TMA_K_SCALE: tl.constexpr=False, PAGE_METADATA_K_SCALE: tl.constexpr=False, PRECOMBINE_Q_SCALE: tl.constexpr=False, KS_STRIDE_BLOCK: tl.constexpr=0, KS_STRIDE_TOKEN: tl.constexpr=0, KS_STRIDE_HEAD: tl.constexpr=0, KS_STRIDE_D: tl.constexpr=0, FULL_VIEW_V_RS: tl.constexpr=False, MERGE_CLUSTER_SIZE: tl.constexpr=8, EXECUTION_STAGE: tl.constexpr=_compute_mtp2__EXECUTION_FULL, MAX_FINAL_CHUNKS: tl.constexpr=16, PAIRED_HEAD_FINALIZE: tl.constexpr=False, DETERMINISTIC_TAIL_ELECTION: tl.constexpr=True, RANK0_ONLY_FINALIZER: tl.constexpr=False, SKIP_TRAILING_FINALIZER_BARRIER: tl.constexpr=True, BF16_DSM: tl.constexpr=False, DEFERRED_NORM: tl.constexpr=False, REDUCTION_ONLY: tl.constexpr=False, ALIGNED_FULL_CHUNK_TOKENS: tl.constexpr=0, FULL_VIEW_DSM: tl.constexpr=False):
    cta = tl.program_id(0)
    cluster_rank = tle.shard_id(mesh, 'cluster_x')
    ...
    final_weight_smem = tle.gpu.alloc(
        ...
        dtype=tl.float32, layout=None, scope=tle.gpu.smem,
    ...
    if REDUCTION_ONLY or has_work:
        ...
        while start < seq_len:
            ...
            if FULL_VIEW_V_RS:
                ...
                v_page_reg_t = tl.reshape(v_bytes, (DV, BLOCK_N)).to(
                    tl.float8e4nv, bitcast=True
                )
            pv = tle.gpu.wgmma(
                v_page_reg_t, p_smem, trans_b=True, out_dtype=tl.float32
            )
            pv = tle.gpu.wgmma_wait(0, pv)
            acc = acc * alpha[None, :] + pv
            m_i = m_new
        ...
        if is_last_cluster:
            if RANK0_ONLY_FINALIZER:
                _compute_mtp2___finalize_owned_heads(
                    SPLIT_OUT, LSE, OUT, final_weight_smem, batch, hkv, 0,
                    group_count, H_Q, HEADS_PER_GROUP, DV,
                    SO_STRIDE_B, SO_STRIDE_C, SO_STRIDE_M, SO_STRIDE_H,
                    LSE_STRIDE_B, LSE_STRIDE_C, LSE_STRIDE_HKV,
                    LSE_STRIDE_M, LSE_STRIDE_HG,
                    O_STRIDE_B, O_STRIDE_M, O_STRIDE_H,
                    MAX_FINAL_CHUNKS, 4, 2,
                )
                _compute_mtp2___finalize_owned_heads(
                    SPLIT_OUT, LSE, OUT, final_weight_smem, batch, hkv, 1,
                    group_count, H_Q, HEADS_PER_GROUP, DV,
                    SO_STRIDE_B, SO_STRIDE_C, SO_STRIDE_M, SO_STRIDE_H,
                    LSE_STRIDE_B, LSE_STRIDE_C, LSE_STRIDE_HKV,
                    LSE_STRIDE_M, LSE_STRIDE_HG,
                ...
                _compute_mtp2___finalize_owned_heads(
                    SPLIT_OUT, LSE, OUT, final_weight_smem, batch, hkv,
                    cluster_rank, group_count, H_Q, HEADS_PER_GROUP, DV,
                    SO_STRIDE_B, SO_STRIDE_C, SO_STRIDE_M, SO_STRIDE_H,
                    LSE_STRIDE_B, LSE_STRIDE_C, LSE_STRIDE_HKV,
                    LSE_STRIDE_M, LSE_STRIDE_HG,
                    O_STRIDE_B, O_STRIDE_M, O_STRIDE_H,
                    MAX_FINAL_CHUNKS, 2, 4,
                )
            elif MERGE_CLUSTER_SIZE == 2:
                for pair_pass in tl.static_range(0, 2):
                    _compute_mtp2___finalize_owned_heads(
                        SPLIT_OUT, LSE, OUT, final_weight_smem, batch, hkv,
                        cluster_rank + pair_pass * 4, group_count,
                        H_Q, HEADS_PER_GROUP, DV,
                        SO_STRIDE_B, SO_STRIDE_C, SO_STRIDE_M, SO_STRIDE_H,
                        LSE_STRIDE_B, LSE_STRIDE_C, LSE_STRIDE_HKV,
                        LSE_STRIDE_M, LSE_STRIDE_HG,
                        O_STRIDE_B, O_STRIDE_M, O_STRIDE_H,
                        MAX_FINAL_CHUNKS, 2, 2,
                    )
            else:
                _compute_mtp2___finalize_owned_heads(
                    SPLIT_OUT, LSE, OUT, final_weight_smem, batch, hkv,
...
def _runtime_mtp1__prepare_decode_workspace(inputs: _runtime_mtp1__DecodeInputs, config: _runtime_mtp1__DecodeConfig | None) -> _runtime_mtp1__DecodeWorkspace:
    ...
    split_out = torch.empty((inputs.num_batch, schedule.partial_slots, _runtime_mtp1__NUM_SEQ_Q, num_head_q, _runtime__HEAD_DIM), dtype=torch.float32, device=device)
...
def _runtime_mtp1__refresh_decode_schedule(inputs: _runtime_mtp1__DecodeInputs, workspace: _runtime_mtp1__DecodeWorkspace, mode: Literal['full', 'tail']) -> None:
...
def _runtime_mtp1__fp8_kvpertensor_decode_mtp1_pure_tle(inputs: _runtime_mtp1__DecodeInputs, workspace: _runtime_mtp1__DecodeWorkspace | None, *, refresh_schedule: Literal['full', 'tail'] | None, paired_head_finalize: bool | None, c2_paired_head_finalize: bool, bf16_dsm: bool, deferred_norm: bool, dsm_election_handoff: bool, deterministic_tail_election: bool, quad_head_two_chunk_finalize: bool, rank0_only_finalizer: bool, skip_trailing_finalizer_barrier: bool, reduction_only: bool, aligned_full_chunk: bool, tma_k_scale: bool, page_metadata_k_scale: bool, precombine_q_scale: bool, flatten_output: bool) -> torch.Tensor:
    ...
    ws = workspace
    num_head_q = int(inputs.q.shape[1])
    _compute_mtp1__fp8_kvpertensor_decode_mtp1_final_kernel[ws.schedule.num_clusters,](ws.q_4d, k_desc, ks_desc, v_desc, inputs.block_ids, ws.schedule.task_map, ws.q_scale_3d, inputs.k_scale, inputs.v_scale, ws.split_out, ws.lse, ws.completion, ws.last_flags, ws.out, mesh=_runtime__CLUSTER_MESHES[ws.config.cluster_size], B=inputs.num_batch, H_Q=num_head_q, HEADS_PER_GROUP=ws.heads_per_group, D=_runtime__HEAD_DIM, DV=_runtime__HEAD_DIM, BLOCK_SIZE=_runtime__BLOCK_SIZE, MAX_BLOCKS=inputs.block_ids.shape[1], BLOCK_N=_runtime__TILE_N, Q_STRIDE_B=ws.q_4d.stride(0), Q_STRIDE_M=ws.q_4d.stride(1), Q_STRIDE_H=ws.q_4d.stride(2), QS_STRIDE_B=ws.q_scale_3d.stride(0), QS_STRIDE_M=ws.q_scale_3d.stride(1), QS_STRIDE_H=ws.q_scale_3d.stride(2), SO_STRIDE_B=ws.split_out.stride(0), SO_STRIDE_C=ws.split_out.stride(1), SO_STRIDE_M=ws.split_out.stride(2), SO_STRIDE_H=ws.split_out.stride(3), LSE_STRIDE_B=ws.lse.stride(0), LSE_STRIDE_C=ws.lse.stride(1), LSE_STRIDE_HKV=ws.lse.stride(2), LSE_STRIDE_M=ws.lse.stride(3), LSE_STRIDE_HG=ws.lse.stride(4), O_STRIDE_B=ws.out.stride(0), O_STRIDE_M=ws.out.stride(1), O_STRIDE_H=ws.out.stride(2), TMA_K_SCALE=tma_k_scale, PAGE_METADATA_K_SCALE=page_metadata_k_scale, PRECOMBINE_Q_SCALE=precombine_q_scale, KS_STRIDE_BLOCK=ks[0], KS_STRIDE_TOKEN=ks[1], KS_STRIDE_HEAD=ks[2], KS_STRIDE_D=ks[3], LDSM_REGISTER_SHARED=True, FULL_VIEW_V_RS=False, MERGE_CLUSTER_SIZE=ws.config.cluster_size, EXECUTION_STAGE=_compute_mtp1__EXECUTION_FULL, MAX_FINAL_CHUNKS=triton.next_power_of_2(ws.schedule.partial_slots), PAIRED_HEAD_FINALIZE=paired_head_finalize, C2_PAIRED_HEAD_FINALIZE=c2_paired_head_finalize, BF16_DSM=bf16_dsm, DEFERRED_NORM=deferred_norm, DSM_ELECTION_HANDOFF=dsm_election_handoff, DETERMINISTIC_TAIL_ELECTION=deterministic_tail_election, RANK0_ONLY_FINALIZER=rank0_only_finalizer, SKIP_TRAILING_FINALIZER_BARRIER=skip_trailing_finalizer_barrier, REDUCTION_ONLY=reduction_only, ALIGNED_FULL_CHUNK_TOKENS=ws.config.chunk_tokens if aligned_full_chunk else 0, num_ctas=1, num_warps=4, num_stages=3, launch_pdl=False)
    if flatten_output:
        return ws.out.reshape(inputs.num_batch * _runtime_mtp1__NUM_SEQ_Q, num_head_q, _runtime__HEAD_DIM)
    return ws.out

def _runtime_mtp1__fp8_kvpertensor_decode_mtp1_final(inputs: _runtime_mtp1__DecodeInputs, workspace: _runtime_mtp1__DecodeWorkspace | None, *, refresh_schedule: Literal['full', 'tail'] | None, paired_head_finalize: bool | None, c2_paired_head_finalize: bool, bf16_dsm: bool | None, deferred_norm: bool, dsm_election_handoff: bool | None, deterministic_tail_election: bool | None, reduction_only: bool | None, aligned_full_chunk: bool, full_view_v_rs: bool, skip_trailing_finalizer_barrier: bool | None, tma_k_scale: bool | None, page_metadata_k_scale: bool, precombine_q_scale: bool | None, flatten_output: bool) -> torch.Tensor:
    ...
    auto_aligned_full_chunk = workspace.all_chunks_aligned and aligned_split_only and ((workspace.config.cluster_size, workspace.config.chunk_tokens) in ((2, 256), (2, 1024), (4, 256), (4, 512), (4, 1024), (8, 512)))
    ...
    return _runtime_mtp1__fp8_kvpertensor_decode_mtp1_pure_tle(inputs, workspace, refresh_schedule=refresh_schedule, paired_head_finalize=paired_head_finalize, c2_paired_head_finalize=c2_paired_head_finalize, bf16_dsm=bf16_dsm, deferred_norm=deferred_norm, dsm_election_handoff=dsm_election_handoff, deterministic_tail_election=deterministic_tail_election, reduction_only=reduction_only, aligned_full_chunk=aligned_full_chunk, skip_trailing_finalizer_barrier=skip_trailing_finalizer_barrier, tma_k_scale=tma_k_scale, page_metadata_k_scale=page_metadata_k_scale, precombine_q_scale=precombine_q_scale, flatten_output=flatten_output, quad_head_two_chunk_finalize=False, rank0_only_finalizer=False)

def _runtime_mtp1__fp8_kvpertensor_decode_mtp1_c2_raw_specialized(inputs: _runtime_mtp1__DecodeInputs, workspace: _runtime_mtp1__DecodeWorkspace, *, reduction_only: bool, aligned_full_chunk: bool, tma_k_scale: bool, page_metadata_k_scale: bool, precombine_q_scale: bool) -> torch.Tensor:
    """Run the exact-two C2 BF16/raw MTP=1 path."""
    return _runtime_mtp1__fp8_kvpertensor_decode_mtp1_pure_tle(inputs, workspace, paired_head_finalize=False, c2_paired_head_finalize=False, bf16_dsm=True, deferred_norm=True, quad_head_two_chunk_finalize=True, rank0_only_finalizer=True, skip_trailing_finalizer_barrier=True, reduction_only=reduction_only, aligned_full_chunk=aligned_full_chunk, tma_k_scale=tma_k_scale, page_metadata_k_scale=page_metadata_k_scale, precombine_q_scale=precombine_q_scale, refresh_schedule=None, dsm_election_handoff=False, deterministic_tail_election=False, flatten_output=True)
...
def _runtime_mtp2__prepare_decode_workspace(inputs: _runtime_mtp2__DecodeInputs, config: _runtime_mtp2__DecodeConfig | None) -> _runtime_mtp2__DecodeWorkspace:
    """Allocate buffers and build the complete cluster task map on the GPU."""
    ...
    schedule = _scheduler_mtp24__allocate_cluster_task_map(inputs.kv_lens, num_head_kv=num_head_kv, max_seq_kv=inputs.max_seq_kv, cluster_size=config.cluster_size, chunk_tokens=config.chunk_tokens, direct_threshold=0, subgroup2_threshold=0, num_seq_q=2)
    if schedule.stats is None:
        raise RuntimeError('GPU task scheduler did not publish workspace metadata')
    all_chunks_aligned = bool(torch.all(inputs.kv_lens % config.chunk_tokens == 0).item())
    ...
    pad_heads_per_group = (heads_per_group + 7) // 8 * 8
    device = inputs.q.device
    split_out = torch.empty((inputs.num_batch, schedule.partial_slots, _runtime_mtp2__NUM_SEQ_Q, num_head_q, _runtime__HEAD_DIM), dtype=torch.float32, device=device)
    ...
    return _runtime_mtp2__DecodeWorkspace(config=config, schedule=schedule, q_4d=q_4d, q_scale_3d=q_scale_3d, split_out=split_out, lse=lse, completion=torch.zeros((num_head_kv * inputs.num_batch,), dtype=torch.int32, device=device), last_flags=torch.zeros((schedule.physical_ctas,), dtype=torch.int32, device=device), out=torch.empty((inputs.num_batch, _runtime_mtp2__NUM_SEQ_Q, num_head_q, _runtime__HEAD_DIM), dtype=torch.bfloat16, device=device), heads_per_group=heads_per_group, all_chunks_aligned=all_chunks_aligned)

def _runtime_mtp2__refresh_decode_schedule(inputs: _runtime_mtp2__DecodeInputs, workspace: _runtime_mtp2__DecodeWorkspace, mode: Literal['full', 'tail']) -> None:
    """Refresh an existing GPU schedule when its allocated topology is stable."""
    num_head_kv = int(inputs.k_cache.shape[2])
    if mode == 'full':
...
def _runtime_mtp2__fp8_kvpertensor_decode_mtp2_final(inputs: _runtime_mtp2__DecodeInputs, workspace: _runtime_mtp2__DecodeWorkspace | None, *, refresh_schedule: Literal['full', 'tail'] | None, reuse_final_weights: bool, paired_head_finalize: bool, bf16_dsm: bool, deferred_norm: bool, dsm_election_handoff: bool, deterministic_tail_election: bool, tail_only_election_barrier: bool, reduction_only: bool, aligned_full_chunk: bool, full_view_dsm: bool, full_view_v_rs: bool, quad_head_two_chunk_finalize: bool, rank0_only_finalizer: bool, tma_k_scale: bool, page_metadata_k_scale: bool, precombine_q_scale: bool) -> torch.Tensor:
    ...
    _compute_mtp2__fp8_kvpertensor_decode_mtp2_final_kernel[ws.schedule.num_clusters,](ws.q_4d, k_desc, ks_desc, v_desc, inputs.block_ids, ws.schedule.task_map, ws.q_scale_3d, inputs.k_scale, inputs.v_scale, ws.split_out, ws.lse, ws.completion, ws.out, mesh=_runtime__CLUSTER_MESHES[ws.config.cluster_size], B=inputs.num_batch, H_Q=num_head_q, HEADS_PER_GROUP=ws.heads_per_group, D=_runtime__HEAD_DIM, DV=_runtime__HEAD_DIM, BLOCK_SIZE=_runtime__BLOCK_SIZE, MAX_BLOCKS=inputs.block_ids.shape[1], BLOCK_N=_runtime__TILE_N, Q_STRIDE_B=ws.q_4d.stride(0), Q_STRIDE_M=ws.q_4d.stride(1), Q_STRIDE_H=ws.q_4d.stride(2), QS_STRIDE_B=ws.q_scale_3d.stride(0), QS_STRIDE_M=ws.q_scale_3d.stride(1), QS_STRIDE_H=ws.q_scale_3d.stride(2), SO_STRIDE_B=ws.split_out.stride(0), SO_STRIDE_C=ws.split_out.stride(1), SO_STRIDE_M=ws.split_out.stride(2 if ws.split_out_layout == 'MHD' else 3), SO_STRIDE_H=ws.split_out.stride(3 if ws.split_out_layout == 'MHD' else 2), LSE_STRIDE_B=ws.lse.stride(0), LSE_STRIDE_C=ws.lse.stride(1), LSE_STRIDE_HKV=ws.lse.stride(2), LSE_STRIDE_M=ws.lse.stride(3 if ws.lse_layout == 'MH' else 4), LSE_STRIDE_HG=ws.lse.stride(4 if ws.lse_layout == 'MH' else 3), O_STRIDE_B=ws.out.stride(0), O_STRIDE_M=ws.out.stride(1), O_STRIDE_H=ws.out.stride(2), TMA_K_SCALE=tma_k_scale, PAGE_METADATA_K_SCALE=page_metadata_k_scale, PRECOMBINE_Q_SCALE=precombine_q_scale, KS_STRIDE_BLOCK=ks[0], KS_STRIDE_TOKEN=ks[1], KS_STRIDE_HEAD=ks[2], KS_STRIDE_D=ks[3], FULL_VIEW_V_RS=full_view_v_rs, MERGE_CLUSTER_SIZE=ws.config.cluster_size, EXECUTION_STAGE=stage_map['full'], MAX_FINAL_CHUNKS=triton.next_power_of_2(ws.schedule.partial_slots), PAIRED_HEAD_FINALIZE=paired_head_finalize, DETERMINISTIC_TAIL_ELECTION=deterministic_tail_election, RANK0_ONLY_FINALIZER=rank0_only_finalizer, SKIP_TRAILING_FINALIZER_BARRIER=True, BF16_DSM=bf16_dsm, DEFERRED_NORM=deferred_norm, REDUCTION_ONLY=reduction_only, ALIGNED_FULL_CHUNK_TOKENS=ws.config.chunk_tokens if aligned_full_chunk else 0, FULL_VIEW_DSM=full_view_dsm, num_ctas=1, num_warps=4, num_stages=3, launch_pdl=False)
...
def _runtime_mtp4__prepare_decode_workspace(inputs: _runtime_mtp4__DecodeInputs, config: _runtime_mtp4__DecodeConfig | None, *, split_out_dtype: torch.dtype, raw_scalar_chunk_minor: bool | None) -> _runtime_mtp4__DecodeWorkspace:
    """Allocate buffers and build the complete cluster task map on the GPU."""
    ...
    schedule = _scheduler_mtp24__allocate_cluster_task_map(inputs.kv_lens, num_head_kv=num_head_kv, max_seq_kv=inputs.max_seq_kv, cluster_size=config.cluster_size, chunk_tokens=config.chunk_tokens, direct_threshold=config.direct_threshold, subgroup2_threshold=config.subgroup2_threshold, num_seq_q=_runtime_mtp4__NUM_SEQ_Q)
    if schedule.stats is None:
        raise RuntimeError('GPU task scheduler did not publish workspace metadata')
    all_chunks_aligned = bool(torch.all(inputs.kv_lens % config.chunk_tokens == 0).item())
    ...
    split_out = torch.empty((inputs.num_batch, partial_storage_slots, _runtime_mtp4__NUM_SEQ_Q, num_head_q, _runtime__HEAD_DIM), dtype=split_out_dtype, device=device)
...
def _runtime_mtp4__fp8_kvpertensor_decode_mtp4_c2_bf16_dsm_specialized(inputs: _runtime_mtp4__DecodeInputs, workspace: _runtime_mtp4__DecodeWorkspace, *, tma_k_scale: bool, page_metadata_k_scale: bool, aligned_full_chunk_winner: bool) -> torch.Tensor:
    ...
    return _runtime_mtp4__fp8_kvpertensor_decode_mtp4_pure_tle(inputs, workspace, paired_head_finalize=False, direct_fast_path=True, quad_head_two_chunk_finalize=True, rank0_only_finalizer=True, skip_trailing_finalizer_barrier=True, c4_bf16_dsm=True, c4_deferred_norm=True, full_view_v_rs=True, maxnreg=None, prefetch_k_scale=5 if page_metadata_k_scale else False, tma_k_scale=tma_k_scale, precombine_q_scale=False, c2_aligned_full_chunk_winner=aligned_full_chunk_winner, flatten_output=True, execution_stage='full', fast_finalizer_handoff=False, c4_global_deferred_norm=False, c4_reduction_only_raw=False, c4_aligned_full_chunk_raw=False, pdl_notify=False)
...
def _runtime_mtp4__prepare_mtp4_final_workspace(inputs: _runtime_mtp4__DecodeInputs, config: _runtime_mtp4__DecodeConfig) -> _runtime_mtp4__DecodeWorkspace:
    ...
    """
    workspace = _runtime_mtp4__prepare_decode_workspace(inputs, config, split_out_dtype=torch.float32, raw_scalar_chunk_minor=None)
    resolved = _runtime_mtp4___resolve_mtp4_final_policy(workspace)
    if resolved == 'pdl-s':
        workspace = _runtime_mtp4__prepare_decode_workspace(inputs, config, raw_scalar_chunk_minor=True, split_out_dtype=torch.float32)
...
def _runtime_mtp4__fp8_kvpertensor_decode_mtp4_final(inputs: _runtime_mtp4__DecodeInputs, workspace: _runtime_mtp4__DecodeWorkspace) -> torch.Tensor:
    ...
    if resolved == 'winner':
        ...
        return _runtime_mtp4__fp8_kvpertensor_decode_mtp4_winner(inputs, workspace, prefetch_k_scale=5 if page_metadata_k_scale else winner_tma_k_scale, tma_k_scale=winner_tma_k_scale, precombine_q_scale=winner_tma_k_scale, aligned_full_chunk_winner=aligned_c2)
    if resolved == 'c2-raw':
        ...
        return _runtime_mtp4__fp8_kvpertensor_decode_mtp4_c2_bf16_dsm_specialized(inputs, workspace, tma_k_scale=not page_metadata_k_scale, page_metadata_k_scale=page_metadata_k_scale, aligned_full_chunk_winner=aligned_c2)
    if resolved in ('pdl', 'pdl-s'):
        ...
        return _runtime_mtp4__fp8_kvpertensor_decode_mtp4_detached_raw_finalize(inputs, workspace, maxnreg=producer_maxnreg, page_metadata_k_scale=page_metadata_k_scale, reduction_only_raw=aligned_reduction_only, aligned_full_chunk_raw=aligned_c4, precombine_q_scale=aligned_c4)
    raise ValueError(f'unsupported MTP=4 final policy: {resolved}')

def _dynamic_mtp2__fp8_kvpertensor_decode_mtp2_final(inputs: _runtime_mtp2__DecodeInputs, workspace: _runtime_mtp2__DecodeWorkspace | None, *, refresh_schedule: Literal['full', 'tail'] | None, bf16_dsm: bool | None, deferred_norm: bool, dsm_election_handoff: bool, deterministic_tail_election: bool, tail_only_election_barrier: bool, reduction_only: bool | None, aligned_full_chunk: bool, full_view_dsm: bool, full_view_v_rs: bool, quad_head_two_chunk_finalize: bool, rank0_only_finalizer: bool, skip_trailing_finalizer_barrier: bool | None, tma_k_scale: bool | None, page_metadata_k_scale: bool, precombine_q_scale: bool | None) -> torch.Tensor:
    """Run typed-RS compute with reuse-w on c2/c8 and paired heads on c4."""
    if workspace is None:
        raise ValueError('MTP=2 requires an explicitly configured workspace')
    if tma_k_scale is None:
        tma_k_scale = not page_metadata_k_scale and (not (workspace.config.cluster_size == 4 and workspace.config.chunk_tokens == 128))
    if precombine_q_scale is None:
        precombine_q_scale = True
    aligned_split_only = not any((int(workspace.stats.get(name, 0)) for name in ('direct_tasks', 'dummy_tasks', 'subgroup2_tasks')))
    auto_aligned_full_chunk = (workspace.config.cluster_size, workspace.config.chunk_tokens) in ((2, 256), (2, 512), (2, 1024), (4, 256), (4, 512), (4, 1024)) and workspace.all_chunks_aligned and aligned_split_only
    ...
    if use_c2_raw:
        return _dynamic_mtp2__fp8_kvpertensor_decode_mtp2_c2_raw_specialized(inputs, workspace, reduction_only=aligned_split_only, aligned_full_chunk=aligned_full_chunk or auto_aligned_full_chunk, tma_k_scale=tma_k_scale, page_metadata_k_scale=page_metadata_k_scale, precombine_q_scale=precombine_q_scale)
    if reduction_only is None:
    ...
    return _runtime_mtp2__fp8_kvpertensor_decode_mtp2_final(inputs, workspace, refresh_schedule=refresh_schedule, reuse_final_weights=True, paired_head_finalize=workspace.config.cluster_size == 4, bf16_dsm=bf16_dsm, deferred_norm=deferred_norm, dsm_election_handoff=dsm_election_handoff, deterministic_tail_election=deterministic_tail_election, tail_only_election_barrier=tail_only_election_barrier, reduction_only=reduction_only, aligned_full_chunk=aligned_full_chunk, full_view_dsm=full_view_dsm, full_view_v_rs=full_view_v_rs, quad_head_two_chunk_finalize=quad_head_two_chunk_finalize, rank0_only_finalizer=rank0_only_finalizer, tma_k_scale=tma_k_scale, page_metadata_k_scale=page_metadata_k_scale, precombine_q_scale=precombine_q_scale)

def _dynamic_mtp2__fp8_kvpertensor_decode_mtp2_c2_raw_specialized(inputs: _runtime_mtp2__DecodeInputs, workspace: _runtime_mtp2__DecodeWorkspace, *, reduction_only: bool, aligned_full_chunk: bool, tma_k_scale: bool, page_metadata_k_scale: bool, precombine_q_scale: bool) -> torch.Tensor:
    """Exact MTP4-style C2 BF16/raw path for an isolated MTP2 focused."""
    return _runtime_mtp2__fp8_kvpertensor_decode_mtp2_final(inputs, workspace, reuse_final_weights=False, bf16_dsm=True, deferred_norm=True, full_view_v_rs=True, quad_head_two_chunk_finalize=True, rank0_only_finalizer=True, reduction_only=reduction_only, aligned_full_chunk=aligned_full_chunk, tma_k_scale=tma_k_scale, page_metadata_k_scale=page_metadata_k_scale, precombine_q_scale=precombine_q_scale, refresh_schedule=None, paired_head_finalize=False, dsm_election_handoff=False, deterministic_tail_election=False, tail_only_election_barrier=False, full_view_dsm=False)
...
def _fp8_entry__attention_decode_fp8_tle(inputs: _fp8_entry__FP8DecodeInputs, workspace: _fp8_entry__FP8DecodeWorkspace) -> torch.Tensor:
    ...
    if policy.mtp == 1:
        return _runtime_mtp1__fp8_kvpertensor_decode_mtp1_final(runtime_inputs, runtime_workspace, **{'refresh_schedule': None, 'paired_head_finalize': None, 'c2_paired_head_finalize': False, 'bf16_dsm': None, 'deferred_norm': False, 'dsm_election_handoff': None, 'deterministic_tail_election': None, 'reduction_only': None, 'aligned_full_chunk': False, 'full_view_v_rs': False, 'skip_trailing_finalizer_barrier': None, 'tma_k_scale': None, 'page_metadata_k_scale': False, 'precombine_q_scale': None, 'flatten_output': True, **_fp8_entry___dynamic_mtp1_options(policy)})
    if policy.mtp == 2:
        return _dynamic_mtp2__fp8_kvpertensor_decode_mtp2_final(runtime_inputs, runtime_workspace, **{'refresh_schedule': None, 'bf16_dsm': None, 'deferred_norm': False, 'dsm_election_handoff': False, 'deterministic_tail_election': True, 'tail_only_election_barrier': False, 'reduction_only': None, 'aligned_full_chunk': False, 'full_view_dsm': False, 'full_view_v_rs': False, 'quad_head_two_chunk_finalize': False, 'rank0_only_finalizer': False, 'skip_trailing_finalizer_barrier': None, 'tma_k_scale': None, 'page_metadata_k_scale': False, 'precombine_q_scale': None, **_fp8_entry___dynamic_mtp2_options(policy)})
