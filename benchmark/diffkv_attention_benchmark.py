...
def _fa3_extension_candidates(path: str | None):
    """Yield FA3 shared libraries from a file or extension directory."""
...
def load_fa3_provider(requested: str, extension_path: str | None):
    """Load the optional FA3 CUDA extension and return a status dictionary.

    FA3 is deliberately loaded lazily after argument parsing.  This keeps
    ``--fa3 off`` and non-vLLM installations fully standalone, while allowing
    a vLLM overlay such as ``/home/yuli/fa3-torch210-overlay`` to be measured
    directly in this benchmark process.
    """
    if requested == "off":
        return {
            "available": False,
    ...
    errors: list[str] = []
    candidates = list(_fa3_extension_candidates(extension_path))
    ...
    # If vLLM is installed, importing its flash-attention package may load an
    # extension from its normal installation path.  This is only a fallback;
    # the explicit overlay above remains the reproducible route.
    try:
        import vllm.vllm_flash_attn  # noqa: F401
    except Exception as exc:  # pragma: no cover - optional dependency
        errors.append(f"vLLM import: {type(exc).__name__}: {exc}")
    if _fa3_op_available():
        return {
            "available": True,
            "source": "vllm-import",
    ...
    detail = "; ".join(errors) if errors else "no extension path configured"
    status = {
        "available": False,
        "source": "unavailable",
        "error": detail,
...
def build_fa3_runner(inputs, window_size: int):
    """Build a direct FA3 paged-cache runner with all buffers preallocated."""
    if not _fa3_op_available():
    ...
    # FA3's scheduler metadata selects the optimized Hopper/TMA schedule.
    # vLLM computes this before its timed call as well; omitting it can make
    # an otherwise valid direct custom-op invocation substantially slower.
    ...
    try:
        ...
        # Older FA3 overlays may not expose the scheduler helper.  The fwd
        # custom op remains valid without metadata, albeit on its fallback
        # schedule.
        scheduler_metadata = None

    def run():
        # Keep this positional call in sync with vLLM's FA3 wrapper.  All
        # optional features (rotary, descale, sinks, CP) are disabled here.
        torch.ops._vllm_fa3_C.fwd(
            ...
            None,
            None,
            # vLLM's DiffKV path leaves q_v unset.  FA3 derives the value
            # head dimension from v; supplying q_v is only supported for
            # small (<=64) query head dimensions by the extension.
            None,
            out,
            cu_seqlens_q,
            None,
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "VLLM_FLASH_ATTN_EXTENSION_DIR, e.g. "
            "/home/yuli/fa3-torch210-overlay"
        ),
    )
    ...
    print(
        "FA3 provider: "
        f"requested={args.fa3}, selected={fa3_status['source']}, "
        f"available={fa3_status['available']}"
    )
