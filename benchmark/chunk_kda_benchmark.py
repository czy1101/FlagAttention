...
"""Benchmark chunk Kimi Delta Attention against its torch reference."""
...
from flag_attn.runtime.backend._mthreads.kimi_delta_attention import chunk_kda
...
def _build_inputs(seq_lens, H, D, dtype):
    ...
    initial_state = (
        torch.arange(N * H * D * D, dtype=torch.float32, device=device).reshape(N, H, D, D).to(dtype).float()
    )
...
def parse_args():
    parser = argparse.ArgumentParser(description="Benchmark MThreads chunk_kda")
    parser.add_argument(
        "--seq-lens",
        type=int,
        nargs="+",
        default=None,
        help="run one case with these fixed/packed sequence lengths; default: original cases",
    )
    parser.add_argument("--num-heads", type=int, default=DEFAULT_H)
    parser.add_argument("--head-dim", type=int, choices=(64, 128, 192, 256), default=DEFAULT_D)
...
def benchmark(args, print_output=True):
    if not MUSA_AVAILABLE:
        raise RuntimeError("chunk_kda benchmark requires an available MUSA device")

    dtype = getattr(torch, args.dtype)
    cases = [args.seq_lens] if args.seq_lens is not None else FLASHKDA_CASES
    results = []

    for seq_lens in cases:
        inputs, kwargs = _build_inputs(seq_lens, args.num_heads, args.head_dim, dtype)

        def baseline_run():
            return _chunk_kda_torch_reference_op(*inputs, **kwargs)

        def optimized_run():
            return _chunk_kda_inference_op(*inputs, **kwargs)
        ...
        shape = tuple(seq_lens) + (args.num_heads, args.head_dim)
        result = {
            "shape": shape,
            "dtype": args.dtype,
        ...
        if print_output:
            print(
                f"device=musa shape={shape} dtype={args.dtype} "
                f"latency_ms={latency_ms:.4f} "
                f"baseline=Torch-reference baseline_ms={baseline_ms:.4f} "
