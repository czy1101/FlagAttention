...
from flag_attn.gated_linear_attention.index import prepare_token_indices
from flag_attn.runtime.backend._metax import parallel_nsa
...
def test_parallel(
    ...
    """Compare FlagAttention MetaX parallel_nsa with the naive reference (forward + backward)."""
...
def test_parallel_varlen(
    ...
    """Compare FlagAttention MetaX parallel_nsa with naive reference for variable-length sequences."""
    torch.manual_seed(42)
    os.environ["TRITON_F32_DEFAULT"] = "ieee"
...
def test_parallel_nsa_inputs_unchanged():
    torch.manual_seed(11)

    device = torch.device("cuda")
    B, T, H, HQ, D = 1, 63, 1, 16, 64
    selected_blocks = 16
    block_size = 32
    dtype = torch.float16
    ...
    for name, tensor in named_inputs.items():
        assert torch.equal(tensor, snapshots[name]), (
            f"{name} was modified"
        )
